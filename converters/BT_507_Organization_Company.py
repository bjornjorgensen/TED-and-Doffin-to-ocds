...
def parse_organization_country_subdivision(xml_content):
    ...
    for organization in organizations:
        ...
        country_subentity_code = organization.xpath("efac:Company/cac:PostalAddress/cbc:CountrySubentityCode[@listName='nuts']/text()", namespaces=namespaces)
        
        if org_id and country_subentity_code:
            party_data = {
                "id": org_id[0],
                "address": {
                    "region": country_subentity_code[0]
                }
            }
...
def merge_organization_country_subdivision(release_json, organization_country_subdivision_data):
    ...
    logger.info(f"Merged organization country subdivision data for {len(organization_country_subdivision_data['parties'])} parties")