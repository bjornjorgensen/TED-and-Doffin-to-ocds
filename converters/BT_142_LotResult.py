...
def parse_winner_chosen(xml_content):
    ...
    for lot_result in lot_results:
        ...
        tender_result_code = lot_result.xpath("cbc:TenderResultCode[@listName='winner-selection-status']/text()", namespaces=namespaces)
        ...
        if result_id and tender_result_code and lot_id:
            ...
            if tender_result_code == 'open-nw':
                ...
                award = {
                    "id": result_id,
                    "status": "active",
                    "statusDetails": status_code_mapping.get(tender_result_code, "Unknown"),
                    "relatedLots": [lot_id]
                }
...
def merge_winner_chosen(release_json, winner_chosen_data):
    ...
    release_json["awards"] = [award for award in existing_awards if any(new_award["id"] == award["id"] for new_award in winner_chosen_data["awards"])]

    existing_lots = release_json.setdefault("tender", {}).setdefault("lots", [])
    for new_lot in winner_chosen_data["tender"]["lots"]:
        existing_lot = next((l for l in existing_lots if l["id"] == new_lot["id"]), None)
        if existing_lot:
            existing_lot.update(new_lot)
        else:
            existing_lots.append(new_lot)

    logger.info(f"Merged winner chosen data for {len(winner_chosen_data['awards'])} awards and {len(winner_chosen_data['tender']['lots'])} lots")