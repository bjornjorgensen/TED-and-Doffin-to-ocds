...
def parse_unpublished_subcontracting_value_tender(xml_content):
    ...
    xpath = "//efac:NoticeResult/efac:LotTender/efac:SubcontractingTerm[efbc:TermCode/@listName='applicability']/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='sub-val']"
...
def merge_unpublished_subcontracting_value_tender(release_json, parsed_data):
    if not parsed_data:
        logger.info("No unpublished subcontracting value tender data to merge")
        return

    release_json.setdefault("withheldInformation", []).extend(parsed_data["withheldInformation"])
    logger.info(f"Merged {len(parsed_data['withheldInformation'])} unpublished subcontracting value tender items")