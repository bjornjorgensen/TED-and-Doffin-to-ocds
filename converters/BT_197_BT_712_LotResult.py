...
def parse_bt_197_bt_712_lotresult(xml_content):
    ...
    xpath = "/*/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:NoticeResult/efac:LotResult/efac:AppealRequestsStatistics[efbc:StatisticsCode/@listName='review-type']/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='rev-req']/cbc:ReasonCode"
...
def merge_bt_197_bt_712_lotresult(release_json, bt_197_bt_712_data):
    if not bt_197_bt_712_data:
        logger.warning("No BT-197(BT-712)-LotResult data to merge")
        return
    ...
    for withheld_item in release_json['withheldInformation']:
        if withheld_item.get('id') == "BT-195(BT-712)-LotResult":
            if 'rationaleClassifications' not in withheld_item:
                withheld_item['rationaleClassifications'] = []
            withheld_item['rationaleClassifications'].extend(bt_197_bt_712_data)
            logger.info("Merged BT-197(BT-712)-LotResult data")
            return

    # If no existing BT-712 item found, create a new one
    release_json['withheldInformation'].append({
        "id": "BT-195(BT-712)-LotResult",
        "rationaleClassifications": bt_197_bt_712_data
    })
    logger.info("Created new BT-197(BT-712)-LotResult withheld information item")