...
def parse_award_criterion_number_threshold_lotsgroup(xml_content):
    ...
    for lots_group in lots_groups:
        ...
        award_criteria = lots_group.xpath(".//cac:AwardingCriterion/cac:SubordinateAwardingCriterion", namespaces=namespaces)
        ...
        for criterion in award_criteria:
            threshold_codes = criterion.xpath(".//efac:AwardCriterionParameter/efbc:ParameterCode[@listName='number-threshold']/text()", namespaces=namespaces)
            ...
            for code in threshold_codes:
                threshold_value = "maximumBids" if code == "max-pass" else "minimumScore" if code == "min-score" else None
...
def merge_award_criterion_number_threshold_lotsgroup(release_json, award_criterion_data):
    ...
    for new_group in award_criterion_data["tender"]["lotGroups"]:
        existing_group = next((group for group in tender_lot_groups if group["id"] == new_group["id"]), None)
    ...
    logger.info(f"Merged Award Criterion Number Threshold LotsGroup data for {len(award_criterion_data['tender']['lotGroups'])} lot groups")