# converters/BT_197_BT_543_LotsGroup.py
...
def parse_bt_197_bt_543_lotsgroup(xml_content):
    ...
    xpath = "/*/cac:ProcurementProjectLot[cbc:ID/@schemeName='LotsGroup']/cac:TenderingTerms/cac:AwardingTerms/cac:AwardingCriterion/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='awa-cri-com']/cbc:ReasonCode"
...
def merge_bt_197_bt_543_lotsgroup(release_json, bt_197_bt_543_data):
    if not bt_197_bt_543_data:
        logger.warning("No BT-197(BT-543)-LotsGroup data to merge")
        return
    ...
    for withheld_item in release_json['withheldInformation']:
        if withheld_item.get('id') == "BT-195(BT-543)-LotsGroup":
            if 'rationaleClassifications' not in withheld_item:
                withheld_item['rationaleClassifications'] = []
            withheld_item['rationaleClassifications'].extend(bt_197_bt_543_data)
            logger.info("Merged BT-197(BT-543)-LotsGroup data")
            return

    # If no existing BT-543 item found, create a new one
    release_json['withheldInformation'].append({
        "id": "BT-195(BT-543)-LotsGroup",
        "rationaleClassifications": bt_197_bt_543_data
    })
    logger.info("Created new BT-197(BT-543)-LotsGroup withheld information item")