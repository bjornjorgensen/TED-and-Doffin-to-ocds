...
def parse_bt_197_bt_720_tender(xml_content):
    ...
    xpath = "/*/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:NoticeResult/efac:LotTender/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='win-ten-val']/cbc:ReasonCode"
...
def merge_bt_197_bt_720_tender(release_json, bt_197_bt_720_data):
    if not bt_197_bt_720_data:
        logger.warning("No BT-197(BT-720)-Tender data to merge")
        return
    ...
    for withheld_item in release_json['withheldInformation']:
        if withheld_item.get('id') == "BT-195(BT-720)-Tender":
            if 'rationaleClassifications' not in withheld_item:
                withheld_item['rationaleClassifications'] = []
            withheld_item['rationaleClassifications'].extend(bt_197_bt_720_data)
            logger.info("Merged BT-197(BT-720)-Tender data")
            return

    # If no existing BT-720 item found, create a new one
    release_json['withheldInformation'].append({
        "id": "BT-195(BT-720)-Tender",
        "rationaleClassifications": bt_197_bt_720_data
    })
    logger.info("Created new BT-197(BT-720)-Tender withheld information item")