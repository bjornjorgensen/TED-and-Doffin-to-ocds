...
def parse_contract_conclusion_date(xml_content):
    ...
    for settled_contract in settled_contracts:
        ...
        issue_date = settled_contract.xpath("cbc:IssueDate/text()", namespaces=namespaces)
        
        if contract_id and issue_date:
            contract = {
                "id": contract_id[0],
                "dateSigned": EndDate(issue_date[0])
            }
...
def merge_contract_conclusion_date(release_json, contract_conclusion_date_data):
    ...
    logger.info(f"Merged Contract Conclusion Date data for {len(contract_conclusion_date_data['contracts'])} contracts")