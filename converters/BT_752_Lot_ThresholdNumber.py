...
def parse_selection_criteria_threshold_number(xml_content):
    ...
    for lot in lots:
        ...
        for criterion in selection_criteria:
            ...
            threshold_number = criterion.xpath("efac:CriterionParameter[efbc:ParameterCode/@listName='number-threshold']/efbc:ParameterNumeric/text()", namespaces=namespaces)
            
            if threshold_number:
                criterion_data = {
                    "numbers": [
                        {
                            "number": float(threshold_number[0])
                        }
                    ]
                }
...
def merge_selection_criteria_threshold_number(release_json, threshold_number_data):
    ...
    logger.info(f"Merged selection criteria threshold number data for {len(threshold_number_data['tender']['lots'])} lots")