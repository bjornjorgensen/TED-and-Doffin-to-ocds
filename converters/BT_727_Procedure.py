...
def parse_procedure_place_performance(xml_content):
    ...
    regions = root.xpath("//cac:ProcurementProject/cac:RealizedLocation/cac:Address/cbc:Region/text()", namespaces=namespaces)
...
def merge_procedure_place_performance(release_json, procedure_place_performance_data):
    ...
    logger.info(f"Merged place of performance data for {len(procedure_place_performance_data['tender']['deliveryLocations'])} locations")