...
def parse_place_performance_city(xml_content):
    ...
    for lot in lots:
        ...
        cities = lot.xpath(".//cac:RealizedLocation/cac:Address/cbc:CityName/text()", namespaces=namespaces)
...
def merge_place_performance_city(release_json, place_performance_city_data):
    ...
    for new_item in place_performance_city_data["tender"]["items"]:
        existing_item = next((item for item in tender_items if item["relatedLot"] == new_item["relatedLot"]), None)
    ...
    logger.info(f"Merged Place Performance City data for {len(place_performance_city_data['tender']['items'])} items")