...
def parse_award_criterion_number_fixed_lotsgroup(xml_content):
    ...
    for lot_group in lot_groups:
        ...
        award_criteria = lot_group.xpath(".//cac:AwardingCriterion/cac:SubordinateAwardingCriterion", namespaces=namespaces)
        ...
        for criterion in award_criteria:
            ...
            if fixed_codes:
                criterion_data = {
                    "numbers": [{"fixed": code} for code in fixed_codes]
                }
...
def merge_award_criterion_number_fixed_lotsgroup(release_json, award_criterion_number_fixed_lotsgroup_data):
    if not award_criterion_number_fixed_lotsgroup_data:
        logger.warning("No Award Criterion Number Fixed LotsGroup data to merge")
        return

    tender_lot_groups = release_json.setdefault("tender", {}).setdefault("lotGroups", [])
    
    for new_lot_group in award_criterion_number_fixed_lotsgroup_data["tender"]["lotGroups"]:
        existing_lot_group = next((lg for lg in tender_lot_groups if lg["id"] == new_lot_group["id"]), None)
        ...
        if existing_lot_group:
            ...
            for new_criterion in new_lot_group["awardCriteria"]["criteria"]:
                existing_criterion = next((crit for crit in existing_criteria if crit.get("numbers") == new_criterion["numbers"]), None)
    ...
    logger.info(f"Merged Award Criterion Number Fixed LotsGroup data for {len(award_criterion_number_fixed_lotsgroup_data['tender']['lotGroups'])} lot groups")