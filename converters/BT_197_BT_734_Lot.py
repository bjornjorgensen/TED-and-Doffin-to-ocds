# converters/BT_197_BT_734_Lot.py
...
def parse_bt_197_bt_734_lot(xml_content):
    ...
    xpath = "/*/cac:ProcurementProjectLot[cbc:ID/@schemeName='Lot']/cac:TenderingTerms/cac:AwardingTerms/cac:AwardingCriterion/cac:SubordinateAwardingCriterion/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='awa-cri-nam']/cbc:ReasonCode"
...
def merge_bt_197_bt_734_lot(release_json, bt_197_bt_734_lot_data):
    if not bt_197_bt_734_lot_data:
        logger.warning("No BT-197(BT-734)-Lot data to merge")
        return
    ...
    for withheld_item in release_json['withheldInformation']:
        if withheld_item.get('id') == "BT-195(BT-734)-Lot":
            if 'rationaleClassifications' not in withheld_item:
                withheld_item['rationaleClassifications'] = []
            withheld_item['rationaleClassifications'].extend(bt_197_bt_734_lot_data)
            logger.info("Merged BT-197(BT-734)-Lot data")
            return

    # If no existing BT-734-Lot item found, create a new one
    release_json['withheldInformation'].append({
        "id": "BT-195(BT-734)-Lot",
        "rationaleClassifications": bt_197_bt_734_lot_data
    })
    logger.info("Created new BT-197(BT-734)-Lot withheld information item")