...
def parse_touchpoint_street(xml_content):
    ...
    for organization in organizations:
        ...
        if touchpoint_id:
            street_name = organization.xpath("efac:TouchPoint/cac:PostalAddress/cbc:StreetName/text()", namespaces=namespaces)
            additional_street_name = organization.xpath("efac:TouchPoint/cac:PostalAddress/cbc:AdditionalStreetName/text()", namespaces=namespaces)
            address_lines = organization.xpath("efac:TouchPoint/cac:PostalAddress/cac:AddressLine/cbc:Line/text()", namespaces=namespaces)
...
def merge_touchpoint_street(release_json, touchpoint_street_data):
    if not touchpoint_street_data:
        logger.warning("No TouchPoint Street data to merge")
        return
    ...
    logger.info(f"Merged TouchPoint Street data for {len(touchpoint_street_data['parties'])} parties")