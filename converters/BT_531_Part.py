...
def parse_part_additional_nature(xml_content):
    ...
    additional_natures = root.xpath("//cac:ProcurementProjectLot[cbc:ID/@schemeName='Part']/cac:ProcurementProject/cac:ProcurementAdditionalType[cbc:ProcurementTypeCode/@listName='contract-nature']/cbc:ProcurementTypeCode/text()", namespaces=namespaces)
...
def merge_part_additional_nature(release_json, part_additional_nature_data):
    ...
    new_categories = set(part_additional_nature_data["tender"]["additionalProcurementCategories"])
    ...
    logger.info(f"Merged Part Additional Nature data: {len(combined_categories)} categories")