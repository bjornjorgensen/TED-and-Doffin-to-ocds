...
def parse_bt_196_bt_759_lot_result(xml_content):
    ...
    for lot_result in lot_results:
        ...
        fields_privacy = lot_result.xpath(
            "./efac:ReceivedSubmissionsStatistics/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='rec-sub-cou']",
        ...
        if fields_privacy:
            ...
            if reason_description:
                result["withheldInformation"].append({
                    "id": "rec-sub-cou-" + lot_result.xpath("efbc:ID/text()", namespaces=namespaces)[0],
                    "field": "rec-sub-cou",
                    "name": "Received Submissions Count Justification",
                    "rationale": reason_description[0]
                })

    return result if result["withheldInformation"] else None

def merge_bt_196_bt_759_lot_result(release_json, bt_196_data):
    if not bt_196_data:
        return
