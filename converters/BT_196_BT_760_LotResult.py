...
def parse_bt_196_bt_760_lot_result(xml_content):
    ...
    for lot_result in lot_results:
        ...
        fields_privacy = lot_result.xpath(
            "./efac:ReceivedSubmissionsStatistics/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='rec-sub-typ']",
        ...
        if fields_privacy:
            ...
            if reason_description:
                result["withheldInformation"].append({
                    "id": "rec-sub-typ-" + lot_result.xpath("efbc:ID/text()", namespaces=namespaces)[0],
                    "field": "rec-sub-typ",
                    "name": "Received Submissions Type Justification",
                    "rationale": reason_description[0]
                })

    return result if result["withheldInformation"] else None

def merge_bt_196_bt_760_lot_result(release_json, bt_196_data):
    if not bt_196_data:
        return
