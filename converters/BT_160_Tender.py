...
def parse_concession_revenue_buyer(xml_content):
    ...
    for lot_tender in lot_tenders:
        ...
        revenue_buyer = lot_tender.xpath("efac:ConcessionRevenue/efbc:RevenueBuyerAmount/text()", namespaces=namespaces)
        currency = lot_tender.xpath("efac:ConcessionRevenue/efbc:RevenueBuyerAmount/@currencyID", namespaces=namespaces)
        
        if tender_id and revenue_buyer and currency:
            settled_contract = root.xpath(f"//efac:NoticeResult/efac:SettledContract[efac:LotTender/cbc:ID[@schemeName='tender'] = '{tender_id[0]}']", namespaces=namespaces)
            
            if settled_contract:
                contract_id = settled_contract[0].xpath("cbc:ID[@schemeName='contract']/text()", namespaces=namespaces)
...
def merge_concession_revenue_buyer(release_json, concession_revenue_buyer_data):
    ...
    logger.info(f"Merged Concession Revenue Buyer data for {len(concession_revenue_buyer_data['contracts'])} contracts")