...
def parse_lot_maximum_candidates(xml_content):
    ...
    for lot in lots:
        ...
        max_candidates = lot.xpath("cac:TenderingProcess/cac:EconomicOperatorShortList/cbc:MaximumQuantity/text()", namespaces=namespaces)
        
        if lot_id and max_candidates:
            result["tender"]["lots"].append({
                "id": lot_id[0],
                "secondStage": {
                    "maximumCandidates": int(max_candidates[0])
                }
            })
...
def merge_lot_maximum_candidates(release_json, lot_maximum_candidates_data):
    ...
    for new_lot in lot_maximum_candidates_data["tender"]["lots"]:
        ...
        if existing_lot:
            existing_lot.setdefault("secondStage", {})["maximumCandidates"] = new_lot["secondStage"]["maximumCandidates"]
        else:
