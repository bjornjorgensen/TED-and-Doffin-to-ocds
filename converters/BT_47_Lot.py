...
def parse_participant_name(xml_content):
    ...
    for lot_element in lot_elements:
        ...
        participant_names = lot_element.xpath(".//cac:EconomicOperatorShortList/cac:PreSelectedParty/cac:PartyName/cbc:Name/text()", namespaces=namespaces)
...
def merge_participant_name(release_json, participant_data):
    ...
    for new_lot in participant_data["tender"]["lots"]:
        ...
        if existing_lot:
            ...
            existing_selected_participants = existing_design_contest.setdefault("selectedParticipants", [])
    ...
    logger.info(f"Merged Participant Name data for {len(participant_data['parties'])} parties and {len(participant_data['tender']['lots'])} lots")