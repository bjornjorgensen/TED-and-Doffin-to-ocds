...
def parse_additional_info_deadline_part(xml_content):
    ...
    part_element = root.xpath("//cac:ProcurementProjectLot[cbc:ID/@schemeName='Part']", namespaces=namespaces)
    
    if part_element:
        end_date = part_element[0].xpath("cac:TenderingProcess/cac:AdditionalInformationRequestPeriod/cbc:EndDate/text()", namespaces=namespaces)
        end_time = part_element[0].xpath("cac:TenderingProcess/cac:AdditionalInformationRequestPeriod/cbc:EndTime/text()", namespaces=namespaces)
