...
def parse_unpublished_award_criterion_number_fixed_lot(xml_content):
...
def merge_unpublished_award_criterion_number_fixed_lot(release_json, parsed_data):
    if not parsed_data:
        logger.info("No unpublished award criterion number fixed lot data to merge")
        return

    release_json.setdefault("withheldInformation", []).extend(parsed_data["withheldInformation"])
    logger.info(f"Merged {len(parsed_data['withheldInformation'])} unpublished award criterion number fixed lot items")