...
def parse_bt_197_bt_540_lot(xml_content):
    ...
    xpath = "/*/cac:ProcurementProjectLot[cbc:ID/@schemeName='Lot']/cac:TenderingTerms/cac:AwardingTerms/cac:AwardingCriterion/cac:SubordinateAwardingCriterion/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='awa-cri-des']/cbc:ReasonCode"
...
def merge_bt_197_bt_540_lot(release_json, bt_197_bt_540_data):
    if not bt_197_bt_540_data:
        logger.warning("No BT-197(BT-540)-Lot data to merge")
        return
    ...
    for withheld_item in release_json['withheldInformation']:
        if withheld_item.get('field') == 'BT-540':
            if 'rationaleClassifications' not in withheld_item:
                withheld_item['rationaleClassifications'] = []
            withheld_item['rationaleClassifications'].extend(bt_197_bt_540_data)
            logger.info("Merged BT-197(BT-540)-Lot data")
            return

    # If no existing BT-540 item found, create a new one
    release_json['withheldInformation'].append({
        "field": "BT-540",
        "rationaleClassifications": bt_197_bt_540_data
    })
    logger.info("Created new BT-197(BT-540)-Lot withheld information item")
