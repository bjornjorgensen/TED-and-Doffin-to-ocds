...
def parse_award_criterion_description_lots_group(xml_content):
    ...
    for lot_group in lot_groups:
        ...
        criteria = lot_group.xpath(".//cac:SubordinateAwardingCriterion/cbc:Description/text()", namespaces=namespaces)
...
def merge_award_criterion_description_lots_group(release_json, award_criterion_description_data):
    ...
    for new_lot_group in award_criterion_description_data["tender"]["lotGroups"]:
        ...
        if existing_lot_group:
            ...
            for new_criterion in new_lot_group["awardCriteria"]["criteria"]:
                existing_criterion = next((c for c in existing_criteria if c.get("description") == new_criterion["description"]), None)
    ...
    logger.info(f"Merged Award Criterion Description data for {len(award_criterion_description_data['tender']['lotGroups'])} lot groups")