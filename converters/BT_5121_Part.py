...
def parse_place_performance_post_code_part(xml_content):
    ...
    for part in parts:
        postal_zones = part.xpath("cac:ProcurementProject/cac:RealizedLocation/cac:Address/cbc:PostalZone/text()", namespaces=namespaces)
...
def merge_place_performance_post_code_part(release_json, post_code_data):
    ...
    logger.info(f"Merged Place Performance Post Code (Part) data for {len(post_code_data['tender']['deliveryAddresses'])} addresses")