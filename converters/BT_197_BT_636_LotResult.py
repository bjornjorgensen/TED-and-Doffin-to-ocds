# converters/BT_197_BT_636_LotResult.py
...
def parse_bt_197_bt_636_lotresult(xml_content):
    ...
    xpath = "/*/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:NoticeResult/efac:LotResult/efac:AppealRequestsStatistics[efbc:StatisticsCode/@listName='irregularity-type']/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='buy-rev-typ']/cbc:ReasonCode"
...
def merge_bt_197_bt_636_lotresult(release_json, bt_197_bt_636_data):
    if not bt_197_bt_636_data:
        logger.warning("No BT-197(BT-636)-LotResult data to merge")
        return
    ...
    for withheld_item in release_json['withheldInformation']:
        if withheld_item.get('id') == "BT-195(BT-636)-LotResult":
            if 'rationaleClassifications' not in withheld_item:
                withheld_item['rationaleClassifications'] = []
            withheld_item['rationaleClassifications'].extend(bt_197_bt_636_data)
            logger.info("Merged BT-197(BT-636)-LotResult data")
            return

    # If no existing BT-636 item found, create a new one
    release_json['withheldInformation'].append({
        "id": "BT-195(BT-636)-LotResult",
        "rationaleClassifications": bt_197_bt_636_data
    })
    logger.info("Created new BT-197(BT-636)-LotResult withheld information item")