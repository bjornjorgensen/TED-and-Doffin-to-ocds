    ...
    for lot in lot_elements:
        ...
        if nda_description:
            ...
            result["tender"]["lots"].append({
                "id": lot_id,
                "contractTerms": {
                    "nonDisclosureAgreement": nda_description[0]
                }
            })
        else:
...
def merge_non_disclosure_agreement_description(release_json, nda_description_data):
    ...
    for nda_lot in nda_description_data["tender"]["lots"]:
        ...
        if existing_lot:
            existing_lot.setdefault("contractTerms", {})["nonDisclosureAgreement"] = nda_lot["contractTerms"]["nonDisclosureAgreement"]
            logger.debug(f"Updated NDA description for existing lot {lot_id}")
        else:
            lots.append(nda_lot)
            logger.debug(f"Added new lot {lot_id} with NDA description")

    logger.info(f"Merged NDA descriptions for {len(nda_description_data['tender']['lots'])} lots")