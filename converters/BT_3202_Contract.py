...
def parse_contract_tender_id(xml_content):
    ...
    for notice_result in notice_results:
        ...
        for settled_contract in settled_contracts:
            ...
            lot_tender_id = settled_contract.xpath("efac:LotTender/cbc:ID[@schemeName='tender']/text()", namespaces=namespaces)[0]
            ...
            lot_tender = notice_result.xpath(f"efac:LotTender[cbc:ID[@schemeName='tender']/text()='{lot_tender_id}']", namespaces=namespaces)[0]
            tendering_party_id = lot_tender.xpath("efac:TenderingParty/cbc:ID[@schemeName='tendering-party']/text()", namespaces=namespaces)[0]
            
            tendering_party = notice_result.xpath(f"efac:TenderingParty[cbc:ID[@schemeName='tendering-party']/text()='{tendering_party_id}']", namespaces=namespaces)[0]
            tenderers = tendering_party.xpath("efac:Tenderer", namespaces=namespaces)
            
            for tenderer in tenderers:
                organization_id = tenderer.xpath("cbc:ID[@schemeName='organization']/text()", namespaces=namespaces)[0]
                ...
                lot_results = notice_result.xpath(f"efac:LotResult[efac:SettledContract/cbc:ID[@schemeName='contract']/text()='{contract_id}']", namespaces=namespaces)
...
def merge_contract_tender_id(release_json, contract_tender_id_data):
    ...
    for new_party in contract_tender_id_data["parties"]:
        if new_party["id"] in existing_parties:
            existing_parties[new_party["id"]]["roles"] = list(set(existing_parties[new_party["id"]].get("roles", []) + new_party["roles"]))
        else:
    ...
    # Merge contracts
    existing_contracts = {contract["id"]: contract for contract in release_json.get("contracts", [])}
    for new_contract in contract_tender_id_data["contracts"]:
        if new_contract["id"] in existing_contracts:
            existing_contracts[new_contract["id"]]["relatedBids"] = list(set(existing_contracts[new_contract["id"]].get("relatedBids", []) + new_contract["relatedBids"]))
        else:
            existing_contracts[new_contract["id"]] = new_contract
    release_json["contracts"] = list(existing_contracts.values())

    logger.info(f"Merged Contract Tender ID data for {len(contract_tender_id_data['contracts'])} contracts")