# converters/BT_197_BT_540_Lots_group.py
...
def parse_bt_197_bt_540_lots_group(xml_content):
...
def merge_bt_197_bt_540_lots_group(release_json, bt_197_bt_540_data):
    if not bt_197_bt_540_data:
        logger.warning("No BT-197(BT-540)-LotsGroup data to merge")
        return
    ...
    for withheld_item in release_json['withheldInformation']:
        if withheld_item.get('id') == "BT-195(BT-540)-LotsGroup":
            ...
            logger.info("Merged BT-197(BT-540)-LotsGroup data")
            return

    # If no existing BT-540 item found, create a new one
    release_json['withheldInformation'].append({
        "id": "BT-195(BT-540)-LotsGroup",
        "rationaleClassifications": bt_197_bt_540_data
    })
    logger.info("Created new BT-197(BT-540)-LotsGroup withheld information item")