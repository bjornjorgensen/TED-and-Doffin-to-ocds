# converters/BT_196_BT_553_Tender.py
...
def parse_unpublished_justification_description_tender_bt553(xml_content):
...
def merge_unpublished_justification_description_tender_bt553(release_json, rationale_data):
    ...
    for tender_id, rationale in rationale_data:
        withheld_item = next((item for item in release_json["withheldInformation"] if item.get("id") == f"sub-val-{tender_id}"), None)
        
        if withheld_item is None:
            withheld_item = {"id": f"sub-val-{tender_id}"}
