    ...
    for organization in organizations:
        ...
        if org_id:
            ...
            for ubo in ubos:
                ...
                if ubo_id:
                    street_name = ubo.xpath("cac:ResidenceAddress/cbc:StreetName/text()", namespaces=namespaces)
                    additional_street_name = ubo.xpath("cac:ResidenceAddress/cbc:AdditionalStreetName/text()", namespaces=namespaces)
                    address_lines = ubo.xpath("cac:ResidenceAddress/cac:AddressLine/cbc:Line/text()", namespaces=namespaces)
...
def merge_ubo_streetline2(release_json, ubo_streetline2_data):
    ...
    logger.info(f"Merged UBO Streetline 2 data for {len(ubo_streetline2_data['parties'])} parties")