...
def merge_procedure_place_performance_country_subdivision(release_json, procedure_data):
    if not procedure_data:
        logger.warning("No Procedure Place Performance Country Subdivision data to merge")
        return
    ...
    for new_address in procedure_data["tender"]["deliveryAddresses"]:
        existing_address = next((addr for addr in tender_delivery_addresses if "region" not in addr), None)
        if existing_address:
            existing_address["region"] = new_address["region"]
        else:
            tender_delivery_addresses.append(new_address)

    logger.info(f"Merged Procedure Place Performance Country Subdivision data for {len(procedure_data['tender']['deliveryAddresses'])} addresses")