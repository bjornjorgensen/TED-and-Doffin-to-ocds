...
def parse_contract_modification_description(xml_content):
    ...
    for modification in contract_modifications:
        ...
        reason_description = modification.xpath("efac:ChangeReason/efbc:ReasonDescription/text()", namespaces=namespaces)
        
        if contract_id and reason_description:
            contract = {
                "id": contract_id[0],
                "amendments": [{
                    "id": str(uuid.uuid4()),
    ...
    for new_contract in modification_data["contracts"]:
        ...
        if existing_contract:
            ...
            for new_amendment in new_contract["amendments"]:
                existing_amendment = next((amendment for amendment in existing_amendments if amendment["id"] == new_amendment["id"]), None)
