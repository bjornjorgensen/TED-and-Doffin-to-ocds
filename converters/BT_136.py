...
# Lookup table for direct award justification codes
DIRECT_AWARD_JUSTIFICATION_CODES = {
...
def parse_direct_award_justification_code(xml_content):
    ...
    justification_code = root.xpath(
        "//cac:TenderingProcess/cac:ProcessJustification[cbc:ProcessReasonCode/@listName='direct-award-justification']/cbc:ProcessReasonCode/text()",
        namespaces=namespaces
    )

    if justification_code:
        code = justification_code[0]
        description = DIRECT_AWARD_JUSTIFICATION_CODES.get(code, "Unknown justification")
        return {
            "tender": {
