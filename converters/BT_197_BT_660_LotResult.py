# converters/BT_197_BT_660_LotResult.py
...
def parse_bt_197_bt_660_lotresult(xml_content):
    ...
    xpath = "/*/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:NoticeResult/efac:LotResult/efac:FrameworkAgreementValues/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='ree-val']/cbc:ReasonCode"
...
def merge_bt_197_bt_660_lotresult(release_json, bt_197_bt_660_data):
    if not bt_197_bt_660_data:
        logger.warning("No BT-197(BT-660)-LotResult data to merge")
        return
    ...
    for withheld_item in release_json['withheldInformation']:
        if withheld_item.get('id') == "BT-195(BT-660)-LotResult":
            if 'rationaleClassifications' not in withheld_item:
                withheld_item['rationaleClassifications'] = []
            withheld_item['rationaleClassifications'].extend(bt_197_bt_660_data)
            logger.info("Merged BT-197(BT-660)-LotResult data")
            return

    # If no existing BT-660 item found, create a new one
    release_json['withheldInformation'].append({
        "id": "BT-195(BT-660)-LotResult",
        "rationaleClassifications": bt_197_bt_660_data
    })
    logger.info("Created new BT-197(BT-660)-LotResult withheld information item")