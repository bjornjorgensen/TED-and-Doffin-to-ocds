...
def parse_lot_security_clearance_description(xml_content):
    ...
    for lot in lots:
        ...
        security_clearance = lot.xpath("cac:TenderingTerms/cac:SecurityClearanceTerm/cbc:Description/text()", namespaces=namespaces)
...
def merge_lot_security_clearance_description(release_json, lot_security_clearance_description_data):
    """
    ...
    for new_lot in lot_security_clearance_description_data["tender"]["lots"]:
        ...
        if existing_lot:
            ...
            existing_lot["otherRequirements"]["securityClearance"] = new_lot["otherRequirements"]["securityClearance"]
        else:
            release_json["tender"]["lots"].append(new_lot)

    logger.info(f"Merged security clearance description data for {len(lot_security_clearance_description_data['tender']['lots'])} lots")