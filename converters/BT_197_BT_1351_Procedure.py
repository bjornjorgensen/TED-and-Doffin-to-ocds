# converters/BT_197_BT_1351_Procedure.py
...
def parse_unpublished_justification_code_procedure_bt1351(xml_content):
    ...
    xpath = "/*/cac:TenderingProcess/cac:ProcessJustification[cbc:ProcessReasonCode/@listName='accelerated-procedure']/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='pro-acc-jus']/cbc:ReasonCode"
...
def merge_unpublished_justification_code_procedure_bt1351(release_json, justification_code):
    if not justification_code:
        return

    withheld_info = next((item for item in release_json.get("withheldInformation", []) if item.get("field") == "pro-acc-jus"), None)
