# converters/BT_197_BT_171_Tender.py
...
def parse_bt_197_bt_171_tender(xml_content):
    ...
    xpath = "/*/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:NoticeResult/efac:LotTender/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='ten-ran']/cbc:ReasonCode"
...
def merge_bt_197_bt_171_tender(release_json, bt_197_bt_171_data):
    if not bt_197_bt_171_data:
        logger.warning("No BT-197(BT-171)-Tender data to merge")
        return
    ...
    for withheld_item in release_json['withheldInformation']:
        if withheld_item.get('field') == 'BT-171':
            if 'rationaleClassifications' not in withheld_item:
                withheld_item['rationaleClassifications'] = []
            withheld_item['rationaleClassifications'].extend(bt_197_bt_171_data)
            logger.info("Merged BT-197(BT-171)-Tender data")
            return

    # If no existing BT-171 item found, create a new one
    release_json['withheldInformation'].append({
        "field": "BT-171",
        "rationaleClassifications": bt_197_bt_171_data
    })
    logger.info("Created new BT-197(BT-171)-Tender withheld information item")
