...
def parse_lot_documents_restricted(xml_content):
    ...
    for lot in lots:
        ...
        document_references = lot.xpath(".//cac:CallForTendersDocumentReference", namespaces=namespaces)
...
def merge_lot_documents_restricted(release_json, lot_documents_restricted_data):
    ...
    logger.info(f"Merged lot documents restricted data for {len(lot_documents_restricted_data['tender']['documents'])} documents")