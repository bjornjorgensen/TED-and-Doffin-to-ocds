...
def parse_procedure_country(xml_content):
    ...
    country_codes = root.xpath("//cac:ProcurementProject/cac:RealizedLocation/cac:Address/cac:Country/cbc:IdentificationCode/text()", namespaces=namespaces)
...
def merge_procedure_country(release_json, procedure_country_data):
    ...
    logger.info(f"Merged Procedure Country data for {len(procedure_country_data['tender']['deliveryAddresses'])} delivery addresses")