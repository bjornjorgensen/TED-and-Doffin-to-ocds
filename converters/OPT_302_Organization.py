...
def parse_beneficial_owner_reference(xml_content):
    ...
    for org in organizations:
        ...
        ubo_id = org.xpath(".//efac:UltimateBeneficialOwner/cbc:ID/text()", namespaces=namespaces)
    ...
    for new_party in bo_reference_data["parties"]:
        ...
        if existing_party:
            ...
            for new_bo in new_party["beneficialOwners"]:
                if not any(bo for bo in existing_party["beneficialOwners"] if bo["id"] == new_bo["id"]):
                    existing_party["beneficialOwners"].append(new_bo)
        else:
            release_json["parties"].append(new_party)

    logger.info(f"Merged Beneficial Owner Reference data for {len(bo_reference_data['parties'])} parties")