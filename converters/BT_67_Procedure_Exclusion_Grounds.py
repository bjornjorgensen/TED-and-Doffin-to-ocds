# converters/BT_67_Procedure_Exclusion_Grounds.py
...
def parse_exclusion_grounds(xml_content):
    ...
    specific_requirements = root.xpath("//cac:TenderingTerms/cac:TendererQualificationRequest/cac:SpecificTendererRequirement", namespaces=namespaces)
    for requirement in specific_requirements:
        exclusion_ground = requirement.xpath("cbc:TendererRequirementTypeCode[@listName='exclusion-ground']/text()", namespaces=namespaces)
        additional_description = requirement.xpath("cbc:Description/text()", namespaces=namespaces)

        if exclusion_ground:
            criterion = {
                "type": exclusion_ground[0],
                "description": EXCLUSION_GROUND_MAPPING.get(exclusion_ground[0], "Unknown exclusion ground")
            }
            if additional_description:
                criterion["description"] += f": {additional_description[0]}"
...
def merge_exclusion_grounds(release_json, exclusion_grounds_data):
    if exclusion_grounds_data:
        ...
        for ground in exclusion_grounds_data:
            existing_criterion = next((c for c in criteria if c.get("type") == ground["type"]), None)
            if existing_criterion:
                existing_criterion["description"] = ground["description"]
            else:
                criteria.append(ground)
