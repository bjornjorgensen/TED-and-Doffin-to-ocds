...
def parse_touchpoint_contact_email(xml_content):
    ...
    for organization in organizations:
        ...
        contact_email = organization.xpath("efac:TouchPoint/cac:Contact/cbc:ElectronicMail/text()", namespaces=namespaces)
        company_id = organization.xpath("efac:Company/cac:PartyLegalEntity/cbc:CompanyID/text()", namespaces=namespaces)
        
        if touchpoint_id and contact_email:
            party = {
                "id": touchpoint_id[0],
                "contactPoint": {
...
def merge_touchpoint_contact_email(release_json, touchpoint_contact_email_data):
    if not touchpoint_contact_email_data:
        logger.warning("No TouchPoint Contact Email data to merge")
        return

    existing_parties = release_json.setdefault("parties", [])
    
    for new_party in touchpoint_contact_email_data["parties"]:
    ...
    logger.info(f"Merged TouchPoint Contact Email data for {len(touchpoint_contact_email_data['parties'])} parties")