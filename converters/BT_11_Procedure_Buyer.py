...
def parse_buyer_legal_type(xml_content):
    ...
    for party in contracting_parties:
        org_id = party.xpath(".//cac:PartyIdentification/cbc:ID[@schemeName='organization']/text()", namespaces=namespaces)
        legal_type = party.xpath(".//cac:ContractingPartyType/cbc:PartyTypeCode[@listName='buyer-legal-type']/text()", namespaces=namespaces)
...
def merge_buyer_legal_type(release_json, buyer_legal_type_data):
    ...
    for new_party in buyer_legal_type_data["parties"]:
        ...
        if existing_party:
            ...
            existing_party["details"]["classifications"].append(new_party["details"]["classifications"][0])
        else:
            release_json["parties"].append(new_party)

    logger.info(f"Merged Buyer Legal Type data for {len(buyer_legal_type_data['parties'])} parties")