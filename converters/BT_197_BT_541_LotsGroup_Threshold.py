# converters/BT_197_BT_541_LotsGroup_Threshold.py
...
def parse_bt_197_bt_541_lotsgroup_threshold(xml_content):
...
def merge_bt_197_bt_541_lotsgroup_threshold(release_json, bt_197_bt_541_data):
    if not bt_197_bt_541_data:
        logger.warning("No BT-197(BT-541)-LotsGroup-Threshold data to merge")
        return
    ...
    for withheld_item in release_json['withheldInformation']:
        if withheld_item.get('id') == "BT-195(BT-541)-LotsGroup-Threshold":
            ...
            logger.info("Merged BT-197(BT-541)-LotsGroup-Threshold data")
            return

    # If no existing BT-541 item found, create a new one
    release_json['withheldInformation'].append({
        "id": "BT-195(BT-541)-LotsGroup-Threshold",
        "rationaleClassifications": bt_197_bt_541_data
    })
    logger.info("Created new BT-197(BT-541)-LotsGroup-Threshold withheld information item")