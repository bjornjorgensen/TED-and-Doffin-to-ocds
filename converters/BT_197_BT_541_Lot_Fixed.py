...
def parse_bt_197_bt_541_lot_fixed(xml_content):
...
def merge_bt_197_bt_541_lot_fixed(release_json, bt_197_bt_541_data):
    if not bt_197_bt_541_data:
        logger.warning("No BT-197(BT-541)-Lot-Fixed data to merge")
        return
    ...
    for withheld_item in release_json['withheldInformation']:
        if withheld_item.get('field') == 'BT-541':
            ...
            logger.info("Merged BT-197(BT-541)-Lot-Fixed data")
            return
    ...
    logger.info("Created new BT-197(BT-541)-Lot-Fixed withheld information item")