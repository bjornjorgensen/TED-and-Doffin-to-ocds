# converters/BT_197_BT_733_LotsGroup.py
...
def parse_bt_197_bt_733_lotsgroup(xml_content):
    ...
    xpath = "/*/cac:ProcurementProjectLot[cbc:ID/@schemeName='LotsGroup']/cac:TenderingTerms/cac:AwardingTerms/cac:AwardingCriterion/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='awa-cri-ord']/cbc:ReasonCode"
...
def merge_bt_197_bt_733_lotsgroup(release_json, bt_197_bt_733_lotsgroup_data):
    if not bt_197_bt_733_lotsgroup_data:
        logger.warning("No BT-197(BT-733)-LotsGroup data to merge")
        return
    ...
    for withheld_item in release_json['withheldInformation']:
        if withheld_item.get('id') == "BT-195(BT-733)-LotsGroup":
            if 'rationaleClassifications' not in withheld_item:
                withheld_item['rationaleClassifications'] = []
            withheld_item['rationaleClassifications'].extend(bt_197_bt_733_lotsgroup_data)
            logger.info("Merged BT-197(BT-733)-LotsGroup data")
            return

    # If no existing BT-733-LotsGroup item found, create a new one
    release_json['withheldInformation'].append({
        "id": "BT-195(BT-733)-LotsGroup",
        "rationaleClassifications": bt_197_bt_733_lotsgroup_data
    })
    logger.info("Created new BT-197(BT-733)-LotsGroup withheld information item")