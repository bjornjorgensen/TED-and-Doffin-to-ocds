...
def parse_lot_quantity(xml_content):
    ...
    for i, lot in enumerate(lots, start=1):
        ...
        quantity = lot.xpath("cac:ProcurementProject/cbc:EstimatedOverallContractQuantity/text()", namespaces=namespaces)
...
def merge_lot_quantity(release_json, lot_quantity_data):
    ...
    logger.info(f"Merged Lot Quantity data for {len(lot_quantity_data['tender']['items'])} items")