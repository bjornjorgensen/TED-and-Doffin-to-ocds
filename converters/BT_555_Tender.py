...
def parse_subcontracting_percentage(xml_content):
    ...
    for lot_tender in lot_tenders:
        ...
        related_lots = lot_tender.xpath("efac:TenderLot/cbc:ID[@schemeName='Lot']/text()", namespaces=namespaces)
        ...
        if tender_id and subcontracting_percentage:
            ...
            bid_data = {
                ...
                "relatedLots": list(set(related_lots))  # Use a set to ensure unique lot IDs
            }
...
def merge_subcontracting_percentage(release_json, subcontracting_data):
    ...
    for new_bid in subcontracting_data["bids"]["details"]:
        ...
        if existing_bid:
            ...
            existing_bid["relatedLots"] = list(set(existing_bid.get("relatedLots", []) + new_bid.get("relatedLots", [])))
        else:
            existing_bids.append(new_bid)

    logger.info(f"BT-555-Tender: Merged subcontracting percentage data for {len(subcontracting_data['bids']['details'])} bids")