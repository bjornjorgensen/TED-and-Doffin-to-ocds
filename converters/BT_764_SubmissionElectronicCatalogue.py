...
def parse_submission_electronic_catalogue(xml_content: str) -> Optional[List[Dict]]:
    ...
    for lot in lots:
        ...
        policy = lot.xpath("cac:TenderingTerms/cac:ContractExecutionRequirement[cbc:ExecutionRequirementCode/@listName='ecatalog-submission']/cbc:ExecutionRequirementCode/text()", namespaces=namespaces)
        ...
        if policy:
            ...
            lots_data.append({
                "id": lot_id,
                "submissionTerms": {
                    "electronicCatalogPolicy": mapped_policy
                }
            })

    return lots_data if lots_data else None

def merge_submission_electronic_catalogue(release_json: Dict, submission_electronic_catalogue_data: Optional[List[Dict]]) -> None:
    """
    ...
    for lot_data in submission_electronic_catalogue_data:
        existing_lot = next((lot for lot in lots if lot.get("id") == lot_data["id"]), None)
        if existing_lot:
            existing_lot.setdefault("submissionTerms", {}).update(lot_data["submissionTerms"])
        else:
            lots.append(lot_data)

    logger.info(f"Merged Submission Electronic Catalogue data for {len(submission_electronic_catalogue_data)} lots")