...
def parse_part_country(xml_content):
    ...
    for part in parts:
        country_codes = part.xpath(".//cac:RealizedLocation/cac:Address/cac:Country/cbc:IdentificationCode/text()", namespaces=namespaces)
...
def merge_part_country(release_json, part_country_data):
    ...
    logger.info(f"Merged Part Country data for {len(part_country_data['tender']['deliveryAddresses'])} delivery addresses")