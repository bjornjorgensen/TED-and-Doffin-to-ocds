...
def parse_procedure_place_performance_additional(xml_content):
    ...
    descriptions = root.xpath("//cac:ProcurementProject/cac:RealizedLocation/cbc:Description/text()", namespaces=namespaces)
...
def merge_procedure_place_performance_additional(release_json, procedure_place_performance_additional_data):
    """
    ...
    if not procedure_place_performance_additional_data:
        logger.warning("No additional procurement procedure place of performance data to merge")
        return
    ...
    for new_address in procedure_place_performance_additional_data["tender"]["deliveryAddresses"]:
    ...
    logger.info(f"Merged additional place of performance data for {len(procedure_place_performance_additional_data['tender']['deliveryAddresses'])} addresses")