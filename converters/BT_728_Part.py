...
def parse_part_place_performance_additional(xml_content):
    ...
    descriptions = root.xpath("//cac:ProcurementProjectLot[cbc:ID/@schemeName='Part']/cac:ProcurementProject/cac:RealizedLocation/cbc:Description/text()", namespaces=namespaces)

    for description in descriptions:
        result["tender"]["deliveryAddresses"].append({"description": description.strip()})

    return result if result["tender"]["deliveryAddresses"] else None

def merge_part_place_performance_additional(release_json, part_place_performance_additional_data):
    """
    ...
    if not part_place_performance_additional_data:
        logger.warning("No additional procurement part place of performance data to merge")
        return
    ...
    for new_address in part_place_performance_additional_data["tender"]["deliveryAddresses"]:
        existing_address = next((addr for addr in release_json["tender"]["deliveryAddresses"] if addr.get("description") == new_address["description"]), None)
        if existing_address:
    ...
    logger.info(f"Merged additional place of performance data for {len(part_place_performance_additional_data['tender']['deliveryAddresses'])} addresses")