# converters/BT_708_Documents_Official_Language.py
...
def parse_documents_official_language(xml_content):
...
def merge_documents_official_language(release_json, language_data):
    if language_data:
        tender = release_json.setdefault("tender", {})
        documents = tender.setdefault("documents", [])

        for doc_data in language_data:
            existing_doc = next((d for d in documents if d.get("id") == doc_data["id"]), None)
            if existing_doc:
                existing_doc["languages"] = doc_data["languages"]
