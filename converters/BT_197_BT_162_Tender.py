# converters/BT_197_BT_162_Tender.py
...
def parse_bt_197_bt_162_tender(xml_content):
    ...
    xpath = "/*/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:NoticeResult/efac:LotTender/efac:ConcessionRevenue/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='con-rev-use']/cbc:ReasonCode"
...
def merge_bt_197_bt_162_tender(release_json, bt_197_bt_162_data):
    if not bt_197_bt_162_data:
        logger.warning("No BT-197(BT-162)-Tender data to merge")
        return
    ...
    for withheld_item in release_json['withheldInformation']:
        if withheld_item.get('field') == 'BT-162':
            if 'rationaleClassifications' not in withheld_item:
                withheld_item['rationaleClassifications'] = []
            withheld_item['rationaleClassifications'].extend(bt_197_bt_162_data)
            logger.info("Merged BT-197(BT-162)-Tender data")
            return

    # If no existing BT-162 item found, create a new one
    release_json['withheldInformation'].append({
        "field": "BT-162",
        "rationaleClassifications": bt_197_bt_162_data
    })
    logger.info("Created new BT-197(BT-162)-Tender withheld information item")
