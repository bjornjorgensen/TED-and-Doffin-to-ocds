...
def parse_bt_197_bt_88_procedure(xml_content):
    ...
    xpath = "/*/cac:TenderingProcess/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='pro-fea']/cbc:ReasonCode"
...
def merge_bt_197_bt_88_procedure(release_json, bt_197_bt_88_procedure_data):
    if not bt_197_bt_88_procedure_data:
        logger.warning("No BT-197(BT-88)-Procedure data to merge")
        return
    ...
    for withheld_item in release_json['withheldInformation']:
        if withheld_item.get('id') == "BT-195(BT-88)-Procedure":
            if 'rationaleClassifications' not in withheld_item:
                withheld_item['rationaleClassifications'] = []
            withheld_item['rationaleClassifications'].extend(bt_197_bt_88_procedure_data)
            logger.info("Merged BT-197(BT-88)-Procedure data")
            return

    # If no existing BT-88-Procedure item found, create a new one
    release_json['withheldInformation'].append({
        "id": "BT-195(BT-88)-Procedure",
        "rationaleClassifications": bt_197_bt_88_procedure_data
    })
    logger.info("Created new BT-197(BT-88)-Procedure withheld information item")