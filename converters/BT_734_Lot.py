...
def parse_award_criterion_name(xml_content):
    ...
    for lot in lots:
        ...
        criteria = lot.xpath(".//cac:SubordinateAwardingCriterion/cbc:Name/text()", namespaces=namespaces)
...
def merge_award_criterion_name(release_json, award_criterion_data):
    ...
    logger.info(f"Merged Award Criterion Name data for {len(award_criterion_data['tender']['lots'])} lots")