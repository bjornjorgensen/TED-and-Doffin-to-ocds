...
def parse_procurement_documents_change_date(xml_content):
    ...
    for change in changes:
        date = change.xpath("efbc:ProcurementDocumentsChangeDate/text()", namespaces=namespaces)
        if not date:
            continue
    ...
    for new_doc in change_date_data["tender"]["documents"]:
        if "relatedLots" in new_doc:
            for lot_id in new_doc["relatedLots"]:
                existing_doc = next((doc for doc in tender_documents if doc.get("documentType") == "biddingDocuments" and lot_id in doc.get("relatedLots", [])), None)
                if existing_doc:
                    existing_doc["dateModified"] = new_doc["dateModified"]
                else:
                    tender_documents.append(new_doc)
        else:
            existing_doc = next((doc for doc in tender_documents if doc.get("documentType") == "biddingDocuments" and "relatedLots" not in doc), None)
            if existing_doc:
                existing_doc["dateModified"] = new_doc["dateModified"]
            else:
                tender_documents.append(new_doc)

    logger.info(f"Merged procurement documents change date data for {len(change_date_data['tender']['documents'])} documents")