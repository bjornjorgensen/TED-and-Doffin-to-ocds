...
def parse_place_performance_post_code(xml_content):
    ...
    for lot in lots:
        ...
        postal_zones = lot.xpath("cac:ProcurementProject/cac:RealizedLocation/cac:Address/cbc:PostalZone/text()", namespaces=namespaces)
        
        if postal_zones:
            item = {
                "id": str(len(result["tender"]["items"]) + 1),
                "deliveryAddresses": [{"postalCode": postal_zone} for postal_zone in postal_zones],
                "relatedLot": lot_id
            }
...
def merge_place_performance_post_code(release_json, post_code_data):
    ...
    logger.info(f"Merged Place Performance Post Code data for {len(post_code_data['tender']['items'])} items")