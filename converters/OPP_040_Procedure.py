...
def parse_main_nature_sub_type(xml_content):
    ...
    additional_types = root.xpath("//cac:ProcurementProject/cac:ProcurementAdditionalType[cbc:ProcurementTypeCode/@listName='transport-service']/cbc:ProcurementTypeCode/text()", namespaces=namespaces)
...
def merge_main_nature_sub_type(release_json, main_nature_sub_type_data):
    ...
    new_categories = main_nature_sub_type_data["tender"]["additionalProcurementCategories"]
    for category in new_categories:
        if category not in existing_categories:
            existing_categories.append(category)

    logger.info(f"Merged Main Nature - Sub Type: added {len(new_categories)} categories")