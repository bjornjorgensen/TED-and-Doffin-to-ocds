...
def parse_lot_public_opening_description(xml_content):
    ...
    for lot in procurement_project_lots:
        ...
        description = lot.xpath("cac:TenderingProcess/cac:OpenTenderEvent/cbc:Description/text()", namespaces=namespaces)
...
def merge_lot_public_opening_description(release_json, lot_public_opening_description_data):
    ...
    logger.debug(f"Release JSON after merging lot public opening description data: {release_json}")
    logger.info(f"Merged Lot Public Opening Description data for {len(lot_public_opening_description_data['tender']['lots'])} lots")