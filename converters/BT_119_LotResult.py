...
def merge_dps_termination(release_json, dps_termination_data):
    ...
    for new_lot in dps_termination_data["lots"]:
        ...
        if existing_lot:
            existing_lot.setdefault("techniques", {})["dynamicPurchasingSystem"] = new_lot["techniques"]["dynamicPurchasingSystem"]
        else:
            existing_lots.append(new_lot)

    logger.info(f"Merged DPS Termination data for {len(dps_termination_data['lots'])} lots")