...
def parse_gpa_coverage(xml_content):
    ...
    for lot in lots:
        ...
        gpa_coverage = lot.xpath(".//cac:TenderingProcess/cbc:GovernmentAgreementConstraintIndicator/text()", namespaces=namespaces)
    ...
    part = root.xpath("//cac:ProcurementProjectLot[cbc:ID/@schemeName='Part']", namespaces=namespaces)
    if part:
        gpa_coverage = part[0].xpath(".//cac:TenderingProcess/cbc:GovernmentAgreementConstraintIndicator/text()", namespaces=namespaces)
        if gpa_coverage and gpa_coverage[0].lower() == 'true':
...
def merge_gpa_coverage(release_json, gpa_coverage_data):
    ...
    if "coveredBy" in gpa_coverage_data["tender"]:
        tender.setdefault("coveredBy", []).extend(gpa_coverage_data["tender"]["coveredBy"])

    logger.info(f"Merged GPA Coverage data for {len(gpa_coverage_data['tender']['lots'])} lots and part")