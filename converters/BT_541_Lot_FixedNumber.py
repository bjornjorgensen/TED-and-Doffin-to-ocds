...
def parse_award_criterion_fixed_number(xml_content):
    ...
    for lot in lots:
        ...
        fixed_numbers = lot.xpath(".//efac:AwardCriterionParameter[efbc:ParameterCode/@listName='number-fixed']/efbc:ParameterNumeric/text()", namespaces=namespaces)
        ...
        if fixed_numbers:
            lot_data = {
                ...
                "awardCriteria": {
                    "criteria": [
                        {
                            "numbers": [{"number": float(number)} for number in fixed_numbers]
                        }
                    ]
                }
            }
...
def merge_award_criterion_fixed_number(release_json, award_criterion_fixed_number_data):
    ...
    logger.info(f"Merged Award Criterion Fixed Number data for {len(award_criterion_fixed_number_data['tender']['lots'])} lots")