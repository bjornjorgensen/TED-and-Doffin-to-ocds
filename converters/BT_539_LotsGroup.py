...
def parse_award_criterion_type_lots_group(xml_content):
    ...
    for lot_group in lot_groups:
        ...
        criteria = lot_group.xpath(".//cac:SubordinateAwardingCriterion/cbc:AwardingCriterionTypeCode[@listName='award-criterion-type']/text()", namespaces=namespaces)
...
def merge_award_criterion_type_lots_group(release_json, award_criterion_type_data):
    ...
    logger.info(f"Merged Award Criterion Type data for {len(award_criterion_type_data['tender']['lotGroups'])} lot groups")