# converters/BT_711_LotResult_Tender_Value_Highest.py
...
def parse_tender_value_highest(xml_content):
    ...
    for lot_result in lot_results:
        ...
        higher_tender_amount = lot_result.xpath("cbc:HigherTenderAmount/text()", namespaces=namespaces)
        currency = lot_result.xpath("cbc:HigherTenderAmount/@currencyID", namespaces=namespaces)

        if lot_id and higher_tender_amount and currency:
            result.append({
                "relatedLot": lot_id[0],
                "value": float(higher_tender_amount[0]),
                "currency": currency[0]
            })

    return result

def merge_tender_value_highest(release_json, tender_value_data):
