...
def parse_place_performance_city_part(xml_content):
    ...
    for part in parts:
        cities = part.xpath(".//cac:RealizedLocation/cac:Address/cbc:CityName/text()", namespaces=namespaces)
...
def merge_place_performance_city_part(release_json, place_performance_city_part_data):
    ...
    logger.info(f"Merged Place Performance City Part data for {len(place_performance_city_part_data['tender']['deliveryAddresses'])} addresses")