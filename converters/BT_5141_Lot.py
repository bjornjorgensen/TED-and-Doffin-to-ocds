...
def parse_lot_country(xml_content):
    ...
    for lot in lots:
        ...
        country_codes = lot.xpath(".//cac:RealizedLocation/cac:Address/cac:Country/cbc:IdentificationCode/text()", namespaces=namespaces)
...
def merge_lot_country(release_json, lot_country_data):
    ...
    logger.info(f"Merged Lot Country data for {len(lot_country_data['tender']['items'])} items")