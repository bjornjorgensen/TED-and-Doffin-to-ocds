...
def parse_employment_legislation_document_reference(xml_content):
    ...
    for lot in lots:
        ...
        document_reference_ids = lot.xpath("cac:TenderingTerms/cac:EmploymentLegislationDocumentReference/cbc:ID/text()", namespaces=namespaces)
        if not document_reference_ids:
            logger.warning("No Employment Legislation Document Reference ID found")
            continue
        document_reference_id = document_reference_ids[0]

        organization_ids = lot.xpath("cac:TenderingTerms/cac:EmploymentLegislationDocumentReference/cac:IssuerParty/cac:PartyIdentification/cbc:ID[@schemeName='organization']/text()", namespaces=namespaces)
...
def merge_employment_legislation_document_reference(release_json, employment_legislation_data):
    if not employment_legislation_data:
        logger.warning("No Employment Legislation Document Reference data to merge")
        return

    # Merge parties
    existing_parties = release_json.setdefault("parties", [])
    
    for new_party in employment_legislation_data.get("parties", []):
    ...
    for new_document in employment_legislation_data["tender"].get("documents", []):
        ...
        if existing_document:
            ...
            existing_document.setdefault("relatedLots", []).extend(
                lot for lot in new_document["relatedLots"] if lot not in existing_document.get("relatedLots", [])
            )
        else:
    ...
    for new_lot in employment_legislation_data["tender"].get("lots", []):
        if not any(lot["id"] == new_lot["id"] for lot in existing_lots):
            existing_lots.append(new_lot)

    logger.info(f"Merged Employment Legislation Document Reference for {len(employment_legislation_data.get('parties', []))} parties")
