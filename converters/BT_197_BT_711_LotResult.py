...
def parse_bt_197_bt_711_lotresult(xml_content):
    ...
    xpath = "/*/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:NoticeResult/efac:LotResult/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='ten-val-hig']/cbc:ReasonCode"
...
def merge_bt_197_bt_711_lotresult(release_json, bt_197_bt_711_data):
    if not bt_197_bt_711_data:
        logger.warning("No BT-197(BT-711)-LotResult data to merge")
        return
    ...
    for withheld_item in release_json['withheldInformation']:
        if withheld_item.get('id') == "BT-195(BT-711)-LotResult":
            if 'rationaleClassifications' not in withheld_item:
                withheld_item['rationaleClassifications'] = []
            withheld_item['rationaleClassifications'].extend(bt_197_bt_711_data)
            logger.info("Merged BT-197(BT-711)-LotResult data")
            return

    # If no existing BT-711 item found, create a new one
    release_json['withheldInformation'].append({
        "id": "BT-195(BT-711)-LotResult",
        "rationaleClassifications": bt_197_bt_711_data
    })
    logger.info("Created new BT-197(BT-711)-LotResult withheld information item")