...
def parse_kilometers_public_transport(xml_content):
    ...
    for lot_tender in lot_tenders:
        ...
        if tender_id and kilometers:
            ...
            settled_contract = root.xpath(f"//efac:NoticeResult/efac:SettledContract[efac:LotTender/cbc:ID[@schemeName='tender']/text()='{tender_id}']", namespaces=namespaces)
...
def merge_kilometers_public_transport(release_json, kilometers_data):
    ...
    logger.info(f"Merged Kilometers Public Transport data for {len(kilometers_data['contracts'])} contracts")