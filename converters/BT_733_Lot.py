...
def parse_lot_award_criteria_order_justification(xml_content):
    ...
    for lot in lots:
        ...
        order_justification = lot.xpath("cac:TenderingTerms/cac:AwardingTerms/cac:AwardingCriterion/cbc:Description/text()", namespaces=namespaces)
...
def merge_lot_award_criteria_order_justification(release_json, lot_award_criteria_order_justification_data):
    """
    ...
    for new_lot in lot_award_criteria_order_justification_data["tender"]["lots"]:
        ...
        if existing_lot:
            ...
            existing_lot["awardCriteria"]["orderRationale"] = new_lot["awardCriteria"]["orderRationale"]
        else:
            release_json["tender"]["lots"].append(new_lot)

    logger.info(f"Merged award criteria order justification data for {len(lot_award_criteria_order_justification_data['tender']['lots'])} lots")