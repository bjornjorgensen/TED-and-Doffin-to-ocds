...
def parse_contract_conditions(xml_content):
    ...
    for lot_tender in lot_tenders:
        ...
        contract_terms = {}
        contract_term_elements = lot_tender.xpath("efac:ContractTerm[efbc:TermCode/@listName='contract-detail']", namespaces=namespaces)
        
        for term in contract_term_elements:
            term_code = term.xpath("efbc:TermCode/text()", namespaces=namespaces)
            term_description = term.xpath("efbc:TermDescription/text()", namespaces=namespaces)
...
def merge_contract_conditions(release_json, contract_conditions_data):
    ...
    logger.info(f"Merged Contract Conditions for {len(contract_conditions_data['tender']['lots'])} lots")