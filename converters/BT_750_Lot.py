...
def merge_selection_criteria(release_json, selection_criteria_data):
    ...
    for new_lot in selection_criteria_data["tender"]["lots"]:
        ...
        if existing_lot:
            ...
            for new_criterion in new_lot["selectionCriteria"]["criteria"]:
                existing_criterion = next((c for c in existing_criteria if c.get("title") == new_criterion.get("title")), None)
    ...
    logger.info(f"Merged selection criteria data for {len(selection_criteria_data['tender']['lots'])} lots")