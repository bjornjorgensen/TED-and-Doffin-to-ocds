...
def parse_lot_prize_value(xml_content: bytes):
    ...
    for lot in lots:
        ...
        prizes = lot.xpath("cac:TenderingTerms/cac:AwardingTerms/cac:Prize", namespaces=namespaces)
        ...
        if prizes:
            ...
            for i, prize in enumerate(prizes):
                value_amount = prize.xpath("cbc:ValueAmount/text()", namespaces=namespaces)
                currency = prize.xpath("cbc:ValueAmount/@currencyID", namespaces=namespaces)
                ...
                if value_amount and currency:
                    try:
                        ...
                        logger.warning(f"Invalid prize value for lot {lot_id}: {value_amount[0]}")
...
def merge_lot_prize_value(release_json, lot_prize_value_data):
    ...
    for new_lot in lot_prize_value_data["tender"]["lots"]:
        ...
        if existing_lot:
            ...
            for new_prize in new_lot["designContest"]["prizes"]["details"]:
                existing_prize = next((prize for prize in details if prize["id"] == new_prize["id"]), None)
                if existing_prize:
    ...
    logger.info(f"Merged lot prize value data for {len(lot_prize_value_data['tender']['lots'])} lots")