...
def parse_buyer_review_complainants(xml_content):
    ...
    for lot_result in lot_results:
        ...
        complainants = lot_result.xpath("efac:AppealRequestsStatistics[efbc:StatisticsCode[@listName='review-type']/text()='complainants']", namespaces=namespaces)
...
def merge_buyer_review_complainants(release_json, buyer_review_complainants_data):
    ...
    for new_statistic in buyer_review_complainants_data["statistics"]:
        existing_statistic = next((stat for stat in existing_statistics if stat.get("measure") == "complainants" and stat.get("relatedLot") == new_statistic["relatedLot"]), None)
