...
def merge_organization_identifier(release_json, organization_identifier_data):
    ...
    for new_party in organization_identifier_data["parties"]:
        ...
        if existing_party:
            existing_party.setdefault("additionalIdentifiers", []).extend(new_party["additionalIdentifiers"])
        else:
            existing_parties.append(new_party)

    logger.info(f"Merged Organization Identifier data for {len(organization_identifier_data['parties'])} parties")