...
def parse_dispatch_invitation_interest(xml_content):
    ...
    for lot in lots:
        ...
        dispatch_date = lot.xpath("cac:TenderingProcess/cac:ParticipationInvitationPeriod/cbc:StartDate/text()", namespaces=namespaces)
...
def merge_dispatch_invitation_interest(release_json, dispatch_invitation_data):
    ...
    logger.info(f"Merged dispatch invitation interest data for {len(dispatch_invitation_data['tender']['lots'])} lots")