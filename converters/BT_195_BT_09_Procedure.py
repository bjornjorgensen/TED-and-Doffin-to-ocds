...
def BT_195_parse_unpublished_identifier_BT_09_Procedure(xml_content):
    ...
    field_privacy = root.xpath("//cac:TenderingTerms/cac:ProcurementLegislationDocumentReference[cbc:ID/text()='CrossBorderLaw']/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='cro-bor-law']/efbc:FieldIdentifierCode", namespaces=namespaces)
...
def BT_195_merge_unpublished_identifier_BT_09_Procedure(release_json, unpublished_identifier_data):
    """
    ...
    for item in unpublished_identifier_data["withheldInformation"]:
        existing_item = next((x for x in withheld_information if x["id"] == item["id"]), None)
        if existing_item:
            existing_item.update(item)
        else:
            withheld_information.append(item)

    logger.info(f"Merged unpublished identifier data: {len(unpublished_identifier_data['withheldInformation'])} items")