...
def merge_contract_url(release_json, contract_url_data):
    ...
    for new_contract in contract_url_data["contracts"]:
        existing_contract = next((c for c in existing_contracts if c["id"] == new_contract["id"]), None)
    ...
    logger.info(f"Merged contract URL data for {len(contract_url_data['contracts'])} contracts")