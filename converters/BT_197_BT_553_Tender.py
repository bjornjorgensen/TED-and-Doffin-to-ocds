...
def parse_bt_197_bt_553_tender(xml_content):
    ...
    xpath = "/*/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:NoticeResult/efac:LotTender/efac:SubcontractingTerm[efbc:TermCode/@listName='applicability']/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='sub-val']/cbc:ReasonCode"
...
def merge_bt_197_bt_553_tender(release_json, bt_197_bt_553_data):
    if not bt_197_bt_553_data:
        logger.warning("No BT-197(BT-553)-Tender data to merge")
        return
    ...
    for withheld_item in release_json['withheldInformation']:
        if withheld_item.get('id') == "BT-195(BT-553)-Tender":
            if 'rationaleClassifications' not in withheld_item:
                withheld_item['rationaleClassifications'] = []
            withheld_item['rationaleClassifications'].extend(bt_197_bt_553_data)
            logger.info("Merged BT-197(BT-553)-Tender data")
            return

    # If no existing BT-553 item found, create a new one
    release_json['withheldInformation'].append({
        "id": "BT-195(BT-553)-Tender",
        "rationaleClassifications": bt_197_bt_553_data
    })
    logger.info("Created new BT-197(BT-553)-Tender withheld information item")