...
def parse_unpublished_justification_code_procedure_bt135(xml_content):
    ...
    xpath = "/*/cac:TenderingProcess/cac:ProcessJustification[cbc:ProcessReasonCode/@listName='direct-award-justification']/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='dir-awa-tex']/cbc:ReasonCode"
...
def merge_unpublished_justification_code_procedure_bt135(release_json, justification_code):
    if not justification_code:
        return

    withheld_info = next((item for item in release_json.get("withheldInformation", []) if item.get("field") == "dir-awa-tex"), None)
