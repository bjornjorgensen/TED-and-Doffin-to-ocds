...
def parse_bt197_bt135_unpublished_justification_code(xml_content):
    """
    ...
    reason_code = root.xpath("//cac:TenderingProcess/cac:ProcessJustification[cbc:ProcessReasonCode/@listName='direct-award-justification']/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='dir-awa-tex']/cbc:ReasonCode/text()", namespaces=namespaces)
...
def merge_bt197_bt135_unpublished_justification_code(release_json, unpublished_justification_code_data):
    """
    ...
    if not unpublished_justification_code_data:
        logger.warning("No unpublished justification code data to merge for BT-197(BT-135)")
        return
    ...
    logger.info("Merged unpublished justification code data for BT-197(BT-135)")