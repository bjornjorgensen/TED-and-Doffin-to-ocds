...
def parse_contract_signatory(xml_content):
    ...
    for signatory_id in signatory_ids:
        ...
        if contract:
            contract_id = contract[0].xpath("cbc:ID[@schemeName='contract']/text()", namespaces=namespaces)[0]
...
def merge_contract_signatory(release_json, signatory_data):
    ...
    existing_awards = {award.get("id"): award for award in release_json.get("awards", [])}
    for award in signatory_data.get("awards", []):
        if award["id"] in existing_awards:
            existing_buyers = {buyer["id"]: buyer for buyer in existing_awards[award["id"]].get("buyers", [])}
    ...
    logger.info(f"Merged Contract Signatory data for {len(signatory_data.get('parties', []))} parties and {len(signatory_data.get('awards', []))} awards")