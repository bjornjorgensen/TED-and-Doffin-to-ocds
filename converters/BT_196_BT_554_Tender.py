...
def parse_unpublished_justification_description_tender_bt554(xml_content):
    ...
    xpath = "/*/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:NoticeResult/efac:LotTender/efac:SubcontractingTerm[efbc:TermCode/@listName='applicability']/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='sub-des']/efbc:ReasonDescription"
...
def merge_unpublished_justification_description_tender_bt554(release_json, rationale_data):
    ...
    for tender_id, rationale in rationale_data:
        withheld_item = next((item for item in release_json["withheldInformation"] if item.get("id") == f"sub-des-{tender_id}"), None)
        
        if withheld_item is None:
            withheld_item = {"id": f"sub-des-{tender_id}"}
