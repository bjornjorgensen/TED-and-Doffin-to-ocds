...
def parse_bt198_bt1252_unpublished_access_date(xml_content):
    """
    ...
    publication_date = root.xpath("//cac:TenderingProcess/cac:ProcessJustification[cbc:ProcessReasonCode/@listName='direct-award-justification']/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='dir-awa-pre']/efbc:PublicationDate/text()", namespaces=namespaces)
...
def merge_bt198_bt1252_unpublished_access_date(release_json, unpublished_access_date_data):
    """
    ...
    if not unpublished_access_date_data:
        logger.warning("No unpublished access date data to merge for BT-198(BT-1252)")
        return
    ...
    logger.info("Merged unpublished access date data for BT-198(BT-1252)")