...
def BT_195_parse_unpublished_procedure_identifier_BT_106(xml_content):
    """
    Parse the XML content to extract the unpublished procedure identifier (BT-195, BT-106).
    ...
    fields_privacy = root.xpath("//cac:TenderingProcess/cac:ProcessJustification[cbc:ProcessReasonCode/@listName='accelerated-procedure']/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='pro-acc']/efbc:FieldIdentifierCode", namespaces=namespaces)

    if fields_privacy:
        withheld_info = {
            "id": f"pro-acc-{contract_folder_id}",
            "field": "pro-acc",
...
def BT_195_merge_unpublished_procedure_identifier_BT_106(release_json, unpublished_procedure_data):
    """
    Merge the parsed unpublished procedure identifier data (BT-195, BT-106) into the main OCDS release JSON.
