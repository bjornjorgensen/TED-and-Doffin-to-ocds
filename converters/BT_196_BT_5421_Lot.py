...
def parse_unpublished_justification_description_lot_bt5421(xml_content):
...
def merge_unpublished_justification_description_lot_bt5421(release_json, rationale_data):
    ...
    for lot_id, rationale in rationale_data:
        withheld_item = next((item for item in release_json["withheldInformation"] if item.get("id") == f"awa-cri-wei-{lot_id}"), None)
        
        if withheld_item is None:
            withheld_item = {"id": f"awa-cri-wei-{lot_id}"}
