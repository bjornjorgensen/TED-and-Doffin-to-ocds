...
def parse_organization_natural_person(xml_content):
    ...
    for organization in organizations:
        ...
        natural_person_indicator = organization.xpath("efbc:NaturalPersonIndicator/text()", namespaces=namespaces)
        
        if org_id and natural_person_indicator and natural_person_indicator[0].lower() == 'true':
            party = {
                "id": org_id[0],
                "details": {
                    "scale": "selfEmployed"
...
def merge_organization_natural_person(release_json, organization_natural_person_data):
    ...
    logger.info(f"Merged Organization Natural Person data for {len(organization_natural_person_data['parties'])} parties")