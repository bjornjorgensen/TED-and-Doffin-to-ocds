...
def parse_bt_197_bt_539_lotsgroup(xml_content):
...
def merge_bt_197_bt_539_lotsgroup(release_json, bt_197_bt_539_data):
    if not bt_197_bt_539_data:
        logger.warning("No BT-197(BT-539)-LotsGroup data to merge")
        return
    ...
    for withheld_item in release_json['withheldInformation']:
        if withheld_item.get('field') == 'BT-539-LotsGroup':
            ...
            logger.info("Merged BT-197(BT-539)-LotsGroup data")
            return

    # If no existing BT-539-LotsGroup item found, create a new one
    release_json['withheldInformation'].append({
        "field": "BT-539-LotsGroup",
        "rationaleClassifications": bt_197_bt_539_data
    })
    logger.info("Created new BT-197(BT-539)-LotsGroup withheld information item")
