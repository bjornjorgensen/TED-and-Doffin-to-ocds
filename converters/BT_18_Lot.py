...
def merge_submission_url(release_json, submission_url_data):
    ...
    for new_lot in submission_url_data["tender"]["lots"]:
        existing_lot = next((lot for lot in existing_lots if lot.get("id") == new_lot["id"]), None)
        if existing_lot:
            existing_lot.update(new_lot)
        else:
            existing_lots.append(new_lot)

    logger.info(f"Merged Submission URL data for {len(submission_url_data['tender']['lots'])} lots")