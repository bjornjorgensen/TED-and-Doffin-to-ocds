...
def parse_ubo_postcode(xml_content):
    ...
    for org in organizations:
        company_id = org.xpath("efac:Organization/efac:Company/cac:PartyIdentification/cbc:ID[@schemeName='organization']/text()", namespaces=namespaces)
        ubo_id = org.xpath("efac:UltimateBeneficialOwner/cbc:ID[@schemeName='ubo']/text()", namespaces=namespaces)
        postal_zone = org.xpath("efac:UltimateBeneficialOwner/cac:ResidenceAddress/cbc:PostalZone/text()", namespaces=namespaces)
...
def merge_ubo_postcode(release_json, ubo_postcode_data):
    ...
    logger.info(f"Merged UBO Postcode data for {len(ubo_postcode_data['parties'])} parties")