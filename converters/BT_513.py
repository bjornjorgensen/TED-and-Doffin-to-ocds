...
def parse_organization_city(xml_content):
    ...
    for org in organizations:
        ...
        if company:
            ...
            city = company[0].xpath("cac:PostalAddress/cbc:CityName/text()", namespaces=namespaces)
            if org_id and city:
                result["parties"].append({
                    "id": org_id[0],
                    "address": {"locality": city[0]}
                })
        ...
        if touchpoint:
            ...
            city = touchpoint[0].xpath("cac:PostalAddress/cbc:CityName/text()", namespaces=namespaces)
    ...
    for ubo in ubos:
        ...
        city = ubo.xpath("cac:ResidenceAddress/cbc:CityName/text()", namespaces=namespaces)
        if ubo_id and city:
...
def merge_organization_city(release_json, city_data):
    if not city_data or "parties" not in city_data:
        return release_json
    ...
    for new_party in city_data["parties"]:
        ...
        if existing_party:
            if "address" not in existing_party:
                existing_party["address"] = {}
            existing_party["address"]["locality"] = new_party["address"]["locality"]
