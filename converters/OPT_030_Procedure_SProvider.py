...
def parse_provided_service_type(xml_content):
    ...
    result = {"parties": []}
    service_providers = root.xpath("//cac:ContractingParty/cac:Party/cac:ServiceProviderParty", namespaces=namespaces)

    for provider in service_providers:
        service_type = provider.xpath("cbc:ServiceTypeCode[@listName='organisation-role']/text()", namespaces=namespaces)
        org_id = provider.xpath("cac:Party/cac:PartyIdentification/cbc:ID/text()", namespaces=namespaces)
...
def merge_provided_service_type(release_json, service_type_data):
    ...
    logger.info(f"Merged Provided Service Type for {len(service_type_data['parties'])} parties")