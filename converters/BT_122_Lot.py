...
def parse_electronic_auction_description(xml_content):
    ...
    for lot in lots:
        ...
        auction_description = lot.xpath("cac:TenderingProcess/cac:AuctionTerms/cbc:Description/text()", namespaces=namespaces)
...
def merge_electronic_auction_description(release_json, auction_description_data):
    ...
    for new_lot in auction_description_data["lots"]:
        ...
        if existing_lot:
            existing_lot.setdefault("techniques", {}).setdefault("electronicAuction", {})["description"] = new_lot["techniques"]["electronicAuction"]["description"]
        else:
            existing_lots.append(new_lot)

    logger.info(f"Merged Electronic Auction Description data for {len(auction_description_data['lots'])} lots")