# converters/BT_196_BT_144_LotResult.py
...
def parse_unpublished_justification_description_lotresult_bt144(xml_content):
    ...
    xpath = "/*/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:NoticeResult/efac:LotResult/efac:DecisionReason/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='no-awa-rea']/efbc:ReasonDescription"
...
def merge_unpublished_justification_description_lotresult_bt144(release_json, rationale_data):
    ...
    for lot_id, rationale in rationale_data:
        withheld_item = next((item for item in release_json["withheldInformation"] if item.get("id") == f"no-awa-rea-{lot_id}"), None)
        
        if withheld_item is None:
            withheld_item = {"id": f"no-awa-rea-{lot_id}"}
