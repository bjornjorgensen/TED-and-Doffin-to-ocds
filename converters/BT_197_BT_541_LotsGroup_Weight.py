# converters/BT_197_BT_541_LotsGroup_Weight.py
...
def parse_bt_197_bt_541_lotsgroup_weight(xml_content):
...
def merge_bt_197_bt_541_lotsgroup_weight(release_json, bt_197_bt_541_data):
    if not bt_197_bt_541_data:
        logger.warning("No BT-197(BT-541)-LotsGroup-Weight data to merge")
        return
    ...
    for withheld_item in release_json['withheldInformation']:
        if withheld_item.get('id') == "BT-195(BT-541)-LotsGroup-Weight":
            ...
            logger.info("Merged BT-197(BT-541)-LotsGroup-Weight data")
            return

    # If no existing BT-541 item found, create a new one
    release_json['withheldInformation'].append({
        "id": "BT-195(BT-541)-LotsGroup-Weight",
        "rationaleClassifications": bt_197_bt_541_data
    })
    logger.info("Created new BT-197(BT-541)-LotsGroup-Weight withheld information item")