...
def parse_bt_197_bt_136_procedure(xml_content):
    ...
    xpath = "/*/cac:TenderingProcess/cac:ProcessJustification[cbc:ProcessReasonCode/@listName='direct-award-justification']/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='dir-awa-jus']/cbc:ReasonCode"
...
def merge_bt_197_bt_136_procedure(release_json, justification_code):
    if not justification_code:
        return

    withheld_info = next((item for item in release_json.get("withheldInformation", []) if item.get("field") == "dir-awa-jus"), None)
    ...
    if withheld_info:
        ...
        release_json["withheldInformation"].append({
            "field": "dir-awa-jus",
            "rationaleClassifications": [justification_code]
        })