...
def parse_unpublished_justification_description_lot_threshold_bt541(xml_content):
...
def merge_unpublished_justification_description_lot_threshold_bt541(release_json, rationale_data):
    ...
    for lot_id, rationale in rationale_data:
        withheld_item = next((item for item in release_json["withheldInformation"] if item.get("id") == f"awa-cri-num-threshold-{lot_id}"), None)
