# tests/test_BT_197_BT_773_Tender.py
...
def test_bt_197_bt_773_tender_integration(tmp_path):
    xml_content = """
    ...
    """
    xml_file = tmp_path / "test_input_bt_197_bt_773_tender.xml"
    ...
    bt_773_tender_item = next((item for item in result["withheldInformation"] if item.get("id") == "BT-195(BT-773)-Tender"), None)
    assert bt_773_tender_item is not None, "No withheld information item for BT-195(BT-773)-Tender found"

    assert "rationaleClassifications" in bt_773_tender_item, "rationaleClassifications not found in withheld information item"
    assert len(bt_773_tender_item["rationaleClassifications"]) > 0, "No rationale classifications found"

    classification = bt_773_tender_item["rationaleClassifications"][0]
