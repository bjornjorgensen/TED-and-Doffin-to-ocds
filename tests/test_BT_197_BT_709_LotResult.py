# tests/test_BT_197_BT_709_LotResult.py
...
def test_bt_197_bt_709_lotresult_integration(tmp_path):
    xml_content = """
    ...
    """
    xml_file = tmp_path / "test_input_bt_197_bt_709_lotresult.xml"
    ...
    bt_709_item = next((item for item in result["withheldInformation"] if item.get("id") == "BT-195(BT-709)-LotResult"), None)
    assert bt_709_item is not None, "No withheld information item for BT-195(BT-709)-LotResult found"

    assert "rationaleClassifications" in bt_709_item, "rationaleClassifications not found in withheld information item"
    assert len(bt_709_item["rationaleClassifications"]) > 0, "No rationale classifications found"

    classification = bt_709_item["rationaleClassifications"][0]
