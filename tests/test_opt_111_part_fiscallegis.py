...
def test_parse_part_fiscal_legislation_document_id(sample_xml):
...
def test_merge_part_fiscal_legislation_document_id():
...
def test_merge_part_fiscal_legislation_document_id_existing_document():
    fiscal_legislation_data = {
        "tender": {"documents": [{"id": "Fiscal1", "documentType": "legislation"}]}
    }
    release_json = {
...
def test_parse_part_fiscal_legislation_document_id_no_data():
    xml_content = "<root></root>"
    result = parse_part_fiscal_legislation_document_id(xml_content)
    assert result is None


def test_merge_part_fiscal_legislation_document_id_no_data():
    release_json = {}
