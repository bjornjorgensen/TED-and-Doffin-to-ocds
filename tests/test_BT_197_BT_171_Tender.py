# tests/test_BT_197_BT_171_Tender.py
...
def test_bt_197_bt_171_tender_integration(tmp_path):
    xml_content = """
    ...
    xml_file = tmp_path / "test_input_bt_197_bt_171_tender.xml"
    ...
    bt_171_item = next((item for item in result["withheldInformation"] if item.get("field") == "BT-171"), None)
    assert bt_171_item is not None, "No withheld information item for BT-171 found"

    assert "rationaleClassifications" in bt_171_item, "rationaleClassifications not found in withheld information item"
    assert len(bt_171_item["rationaleClassifications"]) > 0, "No rationale classifications found"

    classification = bt_171_item["rationaleClassifications"][0]
