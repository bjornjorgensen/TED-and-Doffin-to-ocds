...
def test_bt_196_bt_759_lot_result_integration(tmp_path):
    xml_content = """
                            ...
                            <efac:LotResult>
                                ...
                                <efac:ReceivedSubmissionsStatistics>
                                    <efac:FieldsPrivacy>
                                        ...
                                        <efbc:ReasonDescription>Information delayed publication due to ongoing evaluation process</efbc:ReasonDescription>
                                    </efac:FieldsPrivacy>
                                </efac:ReceivedSubmissionsStatistics>
                            </efac:LotResult>
                        </efac:NoticeResult>
    ...
    """
    xml_file = tmp_path / "test_input_bt_196_bt_759_lot_result.xml"
    ...
    expected_rationale = "Information delayed publication due to ongoing evaluation process"
    assert withheld_info["rationale"] == expected_rationale, f"Expected rationale '{expected_rationale}', got {withheld_info['rationale']}"
    assert withheld_info["id"] == "rec-sub-cou-Lot1", f"Expected id 'rec-sub-cou-Lot1', got {withheld_info['id']}"
    assert withheld_info["field"] == "rec-sub-cou", f"Expected field 'rec-sub-cou', got {withheld_info['field']}"
    assert withheld_info["name"] == "Received Submissions Count Justification", f"Expected name 'Received Submissions Count Justification', got {withheld_info['name']}"
