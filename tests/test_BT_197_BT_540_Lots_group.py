# tests/test_BT_197_BT_540_Lots_group.py
...
def test_bt_197_bt_540_lots_group_integration(tmp_path):
    xml_content = """
    ...
    """
    xml_file = tmp_path / "test_input_bt_197_bt_540_lots_group.xml"
    ...
    bt_540_item = next((item for item in result["withheldInformation"] if item.get("id") == "BT-195(BT-540)-LotsGroup"), None)
    assert bt_540_item is not None, "No withheld information item for BT-195(BT-540)-LotsGroup found"

    assert "rationaleClassifications" in bt_540_item, "rationaleClassifications not found in withheld information item"
    assert len(bt_540_item["rationaleClassifications"]) > 0, "No rationale classifications found"

    classification = bt_540_item["rationaleClassifications"][0]
