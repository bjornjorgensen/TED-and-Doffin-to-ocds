...
def test_bt_197_bt_163_tender_integration(tmp_path):
    xml_content = """
    ...
    xml_file = tmp_path / "test_input_bt_197_bt_163_tender.xml"
    ...
    bt_163_item = next((item for item in result["withheldInformation"] if item.get("field") == "BT-163"), None)
    assert bt_163_item is not None, "No withheld information item for BT-163 found"

    assert "rationaleClassifications" in bt_163_item, "rationaleClassifications not found in withheld information item"
    assert len(bt_163_item["rationaleClassifications"]) > 0, "No rationale classifications found"

    classification = bt_163_item["rationaleClassifications"][0]
