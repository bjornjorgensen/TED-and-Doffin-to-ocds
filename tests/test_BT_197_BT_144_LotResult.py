# tests/test_BT_197_BT_144_LotResult.py
...
def test_bt_197_bt_144_lot_result_integration(tmp_path):
    xml_content = """
    ...
    xml_file = tmp_path / "test_input_bt_197_bt_144_lot_result.xml"
    ...
    bt_144_item = next((item for item in result["withheldInformation"] if item.get("field") == "BT-144"), None)
    assert bt_144_item is not None, "No withheld information item for BT-144 found"

    assert "rationaleClassifications" in bt_144_item, "rationaleClassifications not found in withheld information item"
    assert len(bt_144_item["rationaleClassifications"]) > 0, "No rationale classifications found"

    classification = bt_144_item["rationaleClassifications"][0]
