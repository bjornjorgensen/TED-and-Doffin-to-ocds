# tests/test_BT_197_BT_660_LotResult.py
...
def test_bt_197_bt_660_lotresult_integration(tmp_path):
    xml_content = """
    ...
    """
    xml_file = tmp_path / "test_input_bt_197_bt_660_lotresult.xml"
    ...
    bt_660_item = next((item for item in result["withheldInformation"] if item.get("id") == "BT-195(BT-660)-LotResult"), None)
    assert bt_660_item is not None, "No withheld information item for BT-195(BT-660)-LotResult found"

    assert "rationaleClassifications" in bt_660_item, "rationaleClassifications not found in withheld information item"
    assert len(bt_660_item["rationaleClassifications"]) > 0, "No rationale classifications found"

    classification = bt_660_item["rationaleClassifications"][0]
