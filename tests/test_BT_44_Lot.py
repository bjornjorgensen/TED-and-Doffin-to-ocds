...
def test_bt_44_lot_integration(tmp_path):
    ...
    assert (
        prizes[0]["id"] == "0" and prizes[0]["rank"] == 1
    ), f"Expected first prize with id '0' and rank 1, got {prizes[0]}"
    assert (
        prizes[1]["id"] == "1" and prizes[1]["rank"] == 2
    ), f"Expected second prize with id '1' and rank 2, got {prizes[1]}"
