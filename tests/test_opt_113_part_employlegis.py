...
from src.ted_and_doffin_to_ocds.converters.eforms.opt_113_part_employlegis import (
...
def test_parse_part_employment_legislation_document_id(sample_xml) -> None:
...
def test_merge_part_employment_legislation_document_id() -> None:
...
def test_merge_part_employment_legislation_document_id_existing_document() -> None:
    employment_legislation_data = {
        "tender": {"documents": [{"id": "Empl1", "documentType": "legislation"}]}
    }
    release_json = {
...
def test_parse_part_employment_legislation_document_id_no_data() -> None:
    xml_content = "<root></root>"
    result = parse_part_employment_legislation_document_id(xml_content)
    assert result is None


def test_merge_part_employment_legislation_document_id_no_data() -> None:
    release_json = {}
