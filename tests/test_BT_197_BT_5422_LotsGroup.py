# tests/test_BT_197_BT_5422_LotsGroup.py
...
def test_bt_197_bt_5422_lotsgroup_integration(tmp_path):
    xml_content = """
    ...
    """
    xml_file = tmp_path / "test_input_bt_197_bt_5422_lotsgroup.xml"
    ...
    bt_5422_item = next((item for item in result["withheldInformation"] if item.get("id") == "BT-195(BT-5422)-LotsGroup"), None)
    assert bt_5422_item is not None, "No withheld information item for BT-195(BT-5422)-LotsGroup found"

    assert "rationaleClassifications" in bt_5422_item, "rationaleClassifications not found in withheld information item"
    assert len(bt_5422_item["rationaleClassifications"]) > 0, "No rationale classifications found"

    classification = bt_5422_item["rationaleClassifications"][0]
