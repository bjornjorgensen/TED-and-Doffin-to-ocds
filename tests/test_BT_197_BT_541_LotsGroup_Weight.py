# tests/test_BT_197_BT_541_LotsGroup_Weight.py
...
def test_bt_197_bt_541_lotsgroup_weight_integration(tmp_path):
    xml_content = """
                                    ...
                                    <ext:ExtensionContent>
                                        <efext:EformsExtension>
                                            <efac:AwardCriterionParameter>
                                                <efbc:ParameterCode listName="number-weight">weight-number</efbc:ParameterCode>
                                                <efac:FieldsPrivacy>
    ...
    """
    xml_file = tmp_path / "test_input_bt_197_bt_541_lotsgroup_weight.xml"
    ...
    bt_541_item = next((item for item in result["withheldInformation"] if item.get("id") == "BT-195(BT-541)-LotsGroup-Weight"), None)
    assert bt_541_item is not None, "No withheld information item for BT-195(BT-541)-LotsGroup-Weight found"

    assert "rationaleClassifications" in bt_541_item, "rationaleClassifications not found in withheld information item"
    assert len(bt_541_item["rationaleClassifications"]) > 0, "No rationale classifications found"

    classification = bt_541_item["rationaleClassifications"][0]
