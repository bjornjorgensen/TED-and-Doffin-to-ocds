# tests/test_BT_196_BT_541_LotsGroup_Weight.py
...
def test_bt_196_bt_541_lotsgroup_weight_integration(tmp_path):
    xml_content = """
    ...
    """
    xml_file = tmp_path / "test_input_bt_196_bt_541_lotsgroup_weight.xml"
    ...
    awa_cri_num_weight_item = next((item for item in result["withheldInformation"] if item.get("id") == "awa-cri-num-weight-group-group-1"), None)
    assert awa_cri_num_weight_item is not None, "No withheld information item for awarding criterion weight number (LotsGroup) found"
