# tests/test_BT_197_BT_712_LotResult.py
...
def test_bt_197_bt_712_lotresult_integration(tmp_path):
    xml_content = """
                        ...
                        <efac:NoticeResult>
                            <efac:LotResult>
                                <efac:AppealRequestsStatistics>
                                    <efbc:StatisticsCode listName="review-type">review</efbc:StatisticsCode>
                                    <efac:FieldsPrivacy>
    ...
    """
    xml_file = tmp_path / "test_input_bt_197_bt_712_lotresult.xml"
    ...
    bt_712_item = next((item for item in result["withheldInformation"] if item.get("id") == "BT-195(BT-712)-LotResult"), None)
    assert bt_712_item is not None, "No withheld information item for BT-195(BT-712)-LotResult found"

    assert "rationaleClassifications" in bt_712_item, "rationaleClassifications not found in withheld information item"
    assert len(bt_712_item["rationaleClassifications"]) > 0, "No rationale classifications found"

    classification = bt_712_item["rationaleClassifications"][0]
