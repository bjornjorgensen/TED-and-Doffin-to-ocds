...
def test_bt_144_lotresult_integration(tmp_path):
    xml_content = """
    ...
    assert award["status"] == "unsuccessful", f"Expected status 'unsuccessful', got {award['status']}"
    assert award["statusDetails"] == "No tenders, requests to participate or projects were received", f"Unexpected statusDetails"
