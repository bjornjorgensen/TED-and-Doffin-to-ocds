...
def test_bt_543_lot_integration(tmp_path):
    ...
    assert "weightingDescription" in lot["awardCriteria"], "Expected 'weightingDescription' in lot awardCriteria"
    expected_description = "Price-quality score calculation is based on ..."
    assert lot["awardCriteria"]["weightingDescription"] == expected_description, f"Expected weightingDescription '{expected_description}', got {lot['awardCriteria']['weightingDescription']}"
