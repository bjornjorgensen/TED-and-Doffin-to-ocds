...
def test_bt_196_bt_142_lotresult_multilingual(tmp_path, temp_output_dir) -> None:
    xml_content = """<?xml version="1.0" encoding="UTF-8"?>
    <ContractNotice xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2"
        xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2"
        xmlns:ext="urn:oasis:names:specification:ubl:schema:xsd:CommonExtensionComponents-2"
                    ...
                    <efext:EformsExtension>
                        <efac:NoticeResult>
                            <efac:LotResult>
                                <efac:FieldsPrivacy>
                                    <efbc:FieldIdentifierCode>win-cho</efbc:FieldIdentifierCode>
                                    <efbc:ReasonDescription languageID="ENG">Information delayed publication because of confidentiality</efbc:ReasonDescription>
                                    <efbc:ReasonDescription languageID="NOR">Informasjon utsatt publisering på grunn av konfidensialitet</efbc:ReasonDescription>
                                </efac:FieldsPrivacy>
                                <cbc:ID schemeName="result">RES-0001</cbc:ID>
                            </efac:LotResult>
                        </efac:NoticeResult>
    ...
    """
    xml_file = tmp_path / "test_input_bt196_bt142_multilingual.xml"
    xml_file.write_text(xml_content)
    ...
    # Check primary rationale (should be the English text as it appears first in the XML)
    assert withheld_info["rationale"] == "Information delayed publication because of confidentiality", \
        f"Unexpected primary rationale: {withheld_info['rationale']}"
    
    # Check for the multilingual content
    assert "rationaleMultilingual" in withheld_info, "Expected 'rationaleMultilingual' in withheld_info"
    multilingual_reasons = withheld_info["rationaleMultilingual"]
    
    assert len(multilingual_reasons) == 2, f"Expected 2 language variants, got {len(multilingual_reasons)}"
    
    # Check the English variant
    eng_variant = next((r for r in multilingual_reasons if r.get("languageID") == "ENG"), None)
    assert eng_variant is not None, "English language variant not found"
    assert eng_variant["text"] == "Information delayed publication because of confidentiality", \
        f"Unexpected English text: {eng_variant['text']}"
    
    # Check the Norwegian variant
    nor_variant = next((r for r in multilingual_reasons if r.get("languageID") == "NOR"), None)
    assert nor_variant is not None, "Norwegian language variant not found"
    assert nor_variant["text"] == "Informasjon utsatt publisering på grunn av konfidensialitet", \
        f"Unexpected Norwegian text: {nor_variant['text']}"
