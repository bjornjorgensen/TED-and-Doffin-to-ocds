...
def test_bt_196_bt_554_tender_integration(tmp_path):
    xml_content = """
    ...
    """
    xml_file = tmp_path / "test_input_bt_196_bt_554_tender.xml"
    ...
    sub_des_item = next((item for item in result["withheldInformation"] if item.get("id") == "sub-des-tender-1"), None)
    assert sub_des_item is not None, "No withheld information item for subcontracting description (Tender) found"
    assert "rationale" in sub_des_item, "rationale not found in withheld information item"
    assert sub_des_item["rationale"] == "Information delayed publication because of ...", f"Expected rationale 'Information delayed publication because of ...', got {sub_des_item['rationale']}"
