...
def test_bt_1252_procedure_integration(tmp_path):
    ...
    assert process_1["identifier"] == "123e4567-e89b-12d3-a456-426614174000", "Unexpected identifier for first process"
    assert process_1["scheme"] == "eu-oj", "Expected scheme to be 'eu-oj'"
    assert "unsuccessfulProcess" in process_1["relationship"], "Expected 'unsuccessfulProcess' in first process relationship"

    process_2 = result["relatedProcesses"][1]
    assert process_2["id"] == "2", "Expected second process id to be '2'"
    assert process_2["identifier"] == "234e5678-e89b-12d3-a456-426614174000", "Unexpected identifier for second process"
    assert process_2["scheme"] == "eu-oj", "Expected scheme to be 'eu-oj'"
    assert "prior" in process_2["relationship"], "Expected 'prior' in second process relationship"
