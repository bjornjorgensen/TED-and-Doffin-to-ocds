...
def test_parse_lotsgroup_additional_info():
    ...
    assert result == {'GLO-0001': [{'text': 'For the current procedure ...', 'language': 'ENG'}]}
...
def test_merge_lotsgroup_additional_info():
    ...
    lotsgroup_additional_info = {'GLO-0001': [{'text': 'For the current procedure ...', 'language': 'ENG'}]}
    merge_lotsgroup_additional_info(release_json, lotsgroup_additional_info)
    assert release_json['tender']['lotGroups'][0]['description'] == 'For the current procedure ...'

def test_merge_lotsgroup_additional_info_existing_description():
    release_json = {
        'tender': {
            'lotGroups': [
                {'id': 'GLO-0001', 'description': 'Existing description.'}
            ]
        }
    }
    lotsgroup_additional_info = {'GLO-0001': [{'text': 'Additional info.', 'language': 'ENG'}]}
    merge_lotsgroup_additional_info(release_json, lotsgroup_additional_info)
    assert release_json['tender']['lotGroups'][0]['description'] == 'Existing description. Additional info.'
