# tests/test_BT_196_BT_5422_Lot.py
...
def test_bt_196_bt_5422_lot_integration(tmp_path):
    xml_content = """
    ...
    """
    xml_file = tmp_path / "test_input_bt_196_bt_5422_lot.xml"
    ...
    awa_cri_fix_item = next((item for item in result["withheldInformation"] if item.get("id") == "awa-cri-fix-lot-1"), None)
    assert awa_cri_fix_item is not None, "No withheld information item for awarding criterion fixed number (Lot) found"
