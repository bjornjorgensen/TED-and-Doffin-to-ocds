...
def test_bt_135_136_procedure_integration(tmp_path):
    ...
    expected_rationale = "Specific exclusion justification text Technical reasons justification text"
    assert result["tender"]["procurementMethodRationale"] == expected_rationale, "Unexpected procurementMethodRationale"
    ...
    assert classifications[0]["scheme"] == "eu-direct-award-justification", "Unexpected scheme"
    assert classifications[0]["id"] == "ecom-excl", "Unexpected id"
    assert classifications[0]["description"] == "Specific exclusion in the field of electronic communications", "Unexpected description"
    
    assert classifications[1]["scheme"] == "eu-direct-award-justification", "Unexpected scheme"
    assert classifications[1]["id"] == "technical", "Unexpected id"
    assert classifications[1]["description"] == "The contract can be provided only by a particular economic operator because of an absence of competition for technical reasons", "Unexpected description"
