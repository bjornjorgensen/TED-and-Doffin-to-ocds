...
def test_bt_76_lot_missing_company_legal_form(tmp_path, setup_logging):
    ...
    assert "contractTerms" not in lot or "tendererLegalForm" not in lot.get("contractTerms", {}), \
...
def test_bt_76_lot_multiple_qualification_requests(tmp_path, setup_logging):
    ...
    assert "tendererLegalForm" in lot["contractTerms"], "Expected 'tendererLegalForm' in lot contractTerms"
    assert lot["contractTerms"]["tendererLegalForm"] in ["First legal form requirement", "Second legal form requirement"], \
