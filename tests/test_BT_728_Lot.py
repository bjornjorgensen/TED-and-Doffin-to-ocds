...
def test_bt_728_lot_integration(tmp_path):
    xml_content = """
    ...
    assert len(item["deliveryLocations"]) == 1, f"Expected 1 delivery location, got {len(item['deliveryLocations'])}"
    assert item["deliveryLocations"][0]["description"] == "Further realizations ...", \
