...
def test_bt_197_bt_88_procedure_integration(tmp_path):
    xml_content = """
    ...
    """
    xml_file = tmp_path / "test_input_bt_197_bt_88_procedure.xml"
    ...
    bt_88_procedure_item = next((item for item in result["withheldInformation"] if item.get("id") == "BT-195(BT-88)-Procedure"), None)
    assert bt_88_procedure_item is not None, "No withheld information item for BT-195(BT-88)-Procedure found"

    assert "rationaleClassifications" in bt_88_procedure_item, "rationaleClassifications not found in withheld information item"
    assert len(bt_88_procedure_item["rationaleClassifications"]) > 0, "No rationale classifications found"

    classification = bt_88_procedure_item["rationaleClassifications"][0]
