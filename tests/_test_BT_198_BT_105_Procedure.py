...
def test_bt_198_bt_105_procedure_integration(tmp_path):
    ...
    bt_105_procedure_item = next((item for item in result["withheldInformation"] if item.get("id") == "BT-195(BT-105)-Procedure"), None)
    assert bt_105_procedure_item is not None, "No withheld information item for BT-195(BT-105)-Procedure found"

    assert "availabilityDate" in bt_105_procedure_item, "availabilityDate not found in withheld information item"
    assert bt_105_procedure_item["availabilityDate"] == "2025-03-31T00:00:00+01:00", f"Expected availabilityDate '2025-03-31T00:00:00+01:00', got {bt_105_procedure_item['availabilityDate']}"
