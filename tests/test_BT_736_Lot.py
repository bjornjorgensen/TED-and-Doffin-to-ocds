...
def test_bt_736_lot_integration(tmp_path):
    xml_content = """
    ...
    assert "reservedExecution" in lot["contractTerms"], "Expected 'reservedExecution' in contractTerms"
    assert lot["contractTerms"]["reservedExecution"] is True, "Expected reservedExecution to be True"

    # Check that LOT-0002 is not included
    assert not any(lot["id"] == "LOT-0002" for lot in result["tender"]["lots"]), "LOT-0002 should not be included"
