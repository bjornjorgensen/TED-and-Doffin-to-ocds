...
def test_bt_75_lot_integration(tmp_path, setup_logging, temp_output_dir) -> None:
    ...
    assert (
        "depositsGuarantees" in lot["submissionTerms"]
    ), "Expected 'depositsGuarantees' in submissionTerms"
    expected_description = "Bids shall include a bid security (Provisional Bank Guarantee or bid bond), ..."
    assert (
        lot["submissionTerms"]["depositsGuarantees"] == expected_description
