...
from src.ted_and_doffin_to_ocds.converters.eforms.bt_202_contract import (
    merge_contract_modification_summary,
    parse_contract_modification_summary,
)


def test_parse_contract_modification_summary() -> None:
    xml_content = """
...
def test_merge_contract_modification_summary() -> None:
    ...
    assert "amendments" in contract
    assert len(contract["amendments"]) == 1
    assert contract["amendments"][0]["description"] == "Increase in framework value"
    assert contract["awardID"] == "RES-0001"
