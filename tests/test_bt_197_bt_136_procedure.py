...
def test_bt_197_bt_136_procedure_integration(tmp_path, setup_logging, temp_output_dir):
    ...
    """
    xml_file = tmp_path / "test_input_unpublished_justification_code_bt136.xml"
    ...
    ), "Expected 'rationaleClassifications' in withheld info"
    assert (
...
def test_bt_197_bt_136_procedure_missing_data(tmp_path, setup_logging, temp_output_dir):
    logger = setup_logging
    xml_content = """
    <ContractNotice xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2"
          xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2">
    ...
    """
    xml_file = tmp_path / "test_input_bt_197_bt_136_procedure_missing.xml"
    xml_file.write_text(xml_content)
