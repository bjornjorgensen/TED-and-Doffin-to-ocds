# tests/test_BT_197_BT_733_LotsGroup.py
...
def test_bt_197_bt_733_lotsgroup_integration(tmp_path):
    xml_content = """
    <root xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2"
        ...
        <cac:ProcurementProjectLot>
            <cbc:ID schemeName="LotsGroup">1</cbc:ID>
            <cac:TenderingTerms>
                <cac:AwardingTerms>
    ...
    """
    xml_file = tmp_path / "test_input_bt_197_bt_733_lotsgroup.xml"
    ...
    bt_733_lotsgroup_item = next((item for item in result["withheldInformation"] if item.get("id") == "BT-195(BT-733)-LotsGroup"), None)
    assert bt_733_lotsgroup_item is not None, "No withheld information item for BT-195(BT-733)-LotsGroup found"

    assert "rationaleClassifications" in bt_733_lotsgroup_item, "rationaleClassifications not found in withheld information item"
    assert len(bt_733_lotsgroup_item["rationaleClassifications"]) > 0, "No rationale classifications found"

    classification = bt_733_lotsgroup_item["rationaleClassifications"][0]
