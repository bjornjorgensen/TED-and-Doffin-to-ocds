...
def test_bt_740_procedure_buyer_integration(tmp_path):
    ...
    assert "classifications" in party["details"], "Expected 'classifications' in party details"
    assert len(party["details"]["classifications"]) == 1, f"Expected 1 classification, got {len(party['details']['classifications'])}"

    classification = party["details"]["classifications"][0]
    assert classification["scheme"] == "eu-buyer-contracting-type", f"Expected scheme 'eu-buyer-contracting-type', got {classification['scheme']}"
    assert classification["id"] == "cont-ent", f"Expected id 'cont-ent', got {classification['id']}"
    assert classification["description"] == "Contracting Entity", f"Expected description 'Contracting Entity', got {classification['description']}"
