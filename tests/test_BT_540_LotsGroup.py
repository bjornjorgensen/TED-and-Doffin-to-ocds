...
def test_parse_award_criterion_description_lots_group():
    ...
    assert lot_group["awardCriteria"]["criteria"][0]["description"] == "Tenders with a quality score less than 65..."
...
def test_merge_award_criterion_description_lots_group():
        ...
        "tender": {
            "lotGroups": [
                ...
                },
                {
                    ...
                    "awardCriteria": {
                        "criteria": [
                            {"description": "Another criterion description"}
                        ]
                    }
                }
            ]
    ...
    merge_award_criterion_description_lots_group(release_json, award_criterion_description_data)
    ...
    assert {"description": "Tenders with a quality score less than 65..."} in lot_group1["awardCriteria"]["criteria"]
    
    lot_group2 = release_json["tender"]["lotGroups"][1]
    assert lot_group2["id"] == "GLO-0002"
    assert len(lot_group2["awardCriteria"]["criteria"]) == 1
    assert lot_group2["awardCriteria"]["criteria"][0]["description"] == "Another criterion description"
