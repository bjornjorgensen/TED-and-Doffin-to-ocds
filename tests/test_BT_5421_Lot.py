...
def test_bt_5421_lot_award_criterion_number_weight_integration(tmp_path):
    xml_content = """
    ...
    """
    xml_file = tmp_path / "test_input_lot_award_criterion_number_weight.xml"
    ...
    assert len(lot["awardCriteria"]["criteria"]) == 1, f"Expected 1 criterion, got {len(lot['awardCriteria']['criteria'])}"
    ...
    assert criterion["numbers"][0]["weight"] == "percentageExact", f"Expected weight 'percentageExact', got {criterion['numbers'][0]['weight']}"
