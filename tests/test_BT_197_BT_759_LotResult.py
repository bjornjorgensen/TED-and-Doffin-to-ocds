# tests/test_BT_197_BT_759_LotResult.py
...
def test_bt_197_bt_759_lotresult_integration(tmp_path):
    xml_content = """
    ...
    """
    xml_file = tmp_path / "test_input_bt_197_bt_759_lotresult.xml"
    ...
    bt_759_lotresult_item = next((item for item in result["withheldInformation"] if item.get("id") == "BT-195(BT-759)-LotResult"), None)
    assert bt_759_lotresult_item is not None, "No withheld information item for BT-195(BT-759)-LotResult found"

    assert "rationaleClassifications" in bt_759_lotresult_item, "rationaleClassifications not found in withheld information item"
    assert len(bt_759_lotresult_item["rationaleClassifications"]) > 0, "No rationale classifications found"

    classification = bt_759_lotresult_item["rationaleClassifications"][0]
