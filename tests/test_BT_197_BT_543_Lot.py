...
def test_bt_197_bt_543_lot_integration(tmp_path):
    xml_content = """
    ...
    """
    xml_file = tmp_path / "test_input_bt_197_bt_543_lot.xml"
    ...
    bt_543_item = next((item for item in result["withheldInformation"] if item.get("id") == "BT-195(BT-543)-Lot"), None)
    assert bt_543_item is not None, "No withheld information item for BT-195(BT-543)-Lot found"
