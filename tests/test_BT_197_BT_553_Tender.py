...
def test_bt_197_bt_553_tender_integration(tmp_path):
    xml_content = """
    ...
    """
    xml_file = tmp_path / "test_input_bt_197_bt_553_tender.xml"
    ...
    bt_553_item = next((item for item in result["withheldInformation"] if item.get("id") == "BT-195(BT-553)-Tender"), None)
    assert bt_553_item is not None, "No withheld information item for BT-195(BT-553)-Tender found"

    assert "rationaleClassifications" in bt_553_item, "rationaleClassifications not found in withheld information item"
    assert len(bt_553_item["rationaleClassifications"]) > 0, "No rationale classifications found"

    classification = bt_553_item["rationaleClassifications"][0]
