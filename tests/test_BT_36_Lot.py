...
def test_bt_36_lot_integration(tmp_path):
    ...
    assert lot1["contractPeriod"]["durationInDays"] == 3, f"Expected duration 3 days for LOT-0001, got {lot1['contractPeriod']['durationInDays']}"

    lot2 = next(lot for lot in result["tender"]["lots"] if lot["id"] == "LOT-0002")
    assert lot2["contractPeriod"]["durationInDays"] == 60, f"Expected duration 60 days for LOT-0002, got {lot2['contractPeriod']['durationInDays']}"

    lot3 = next(lot for lot in result["tender"]["lots"] if lot["id"] == "LOT-0003")
    assert lot3["contractPeriod"]["durationInDays"] == 365, f"Expected duration 365 days for LOT-0003, got {lot3['contractPeriod']['durationInDays']}"
