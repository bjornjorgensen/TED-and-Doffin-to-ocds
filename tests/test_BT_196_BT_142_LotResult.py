...
def test_bt_196_bt_142_lotresult_integration(tmp_path):
    ...
    """
    xml_file = tmp_path / "test_input_bt_196_bt_142_lotresult.xml"
    ...
    win_cho_item = next((item for item in result["withheldInformation"] if item.get("id") == "win-cho-lot-1"), None)
    assert win_cho_item is not None, "No withheld information item for winner choice found"
    assert "rationale" in win_cho_item, "rationale not found in withheld information item"
    assert win_cho_item["rationale"] == "Information delayed publication because of ...", f"Expected rationale 'Information delayed publication because of ...', got {win_cho_item['rationale']}"
