# tests/test_BT_197_BT_733_Lot.py
...
def test_bt_197_bt_733_lot_integration(tmp_path):
    xml_content = """
    ...
    """
    xml_file = tmp_path / "test_input_bt_197_bt_733_lot.xml"
    ...
    bt_733_item = next((item for item in result["withheldInformation"] if item.get("id") == "BT-195(BT-733)-Lot"), None)
    assert bt_733_item is not None, "No withheld information item for BT-195(BT-733)-Lot found"

    assert "rationaleClassifications" in bt_733_item, "rationaleClassifications not found in withheld information item"
    assert len(bt_733_item["rationaleClassifications"]) > 0, "No rationale classifications found"

    classification = bt_733_item["rationaleClassifications"][0]
