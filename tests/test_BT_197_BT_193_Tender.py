...
def test_bt_197_bt_193_tender_integration(tmp_path):
    xml_content = """
    ...
    xml_file = tmp_path / "test_input_bt_197_bt_193_tender.xml"
    ...
    bt_193_item = next((item for item in result["withheldInformation"] if item.get("field") == "BT-193"), None)
    assert bt_193_item is not None, "No withheld information item for BT-193 found"

    assert "rationaleClassifications" in bt_193_item, "rationaleClassifications not found in withheld information item"
    assert len(bt_193_item["rationaleClassifications"]) > 0, "No rationale classifications found"

    classification = bt_193_item["rationaleClassifications"][0]
