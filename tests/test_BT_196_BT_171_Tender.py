...
def test_bt196_bt171_unpublished_justification_integration(tmp_path, setup_logging):
...
def test_bt196_bt171_unpublished_justification_missing_data(tmp_path, setup_logging):
    logger = setup_logging
    xml_content = """
    <root xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2"
          xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2"
    ...
    """
    xml_file = tmp_path / "test_input_bt196_bt171_missing.xml"
    xml_file.write_text(xml_content)
