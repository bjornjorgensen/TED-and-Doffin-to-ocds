# tests/test_BT_197_BT_539_LotsGroup.py
...
def test_bt_197_bt_539_lotsgroup_integration(tmp_path):
    xml_content = """
    ...
    xml_file = tmp_path / "test_input_bt_197_bt_539_lotsgroup.xml"
    ...
    bt_539_lotsgroup_item = next((item for item in result["withheldInformation"] if item.get("field") == "BT-539-LotsGroup"), None)
    assert bt_539_lotsgroup_item is not None, "No withheld information item for BT-539-LotsGroup found"

    assert "rationaleClassifications" in bt_539_lotsgroup_item, "rationaleClassifications not found in withheld information item"
    assert len(bt_539_lotsgroup_item["rationaleClassifications"]) > 0, "No rationale classifications found"

    classification = bt_539_lotsgroup_item["rationaleClassifications"][0]
