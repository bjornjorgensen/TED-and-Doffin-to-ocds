...
def test_bt_197_bt_710_lotresult_integration(tmp_path):
    xml_content = """
    ...
    """
    xml_file = tmp_path / "test_input_bt_197_bt_710_lotresult.xml"
    ...
    bt_710_item = next((item for item in result["withheldInformation"] if item.get("id") == "BT-195(BT-710)-LotResult"), None)
    assert bt_710_item is not None, "No withheld information item for BT-195(BT-710)-LotResult found"

    assert "rationaleClassifications" in bt_710_item, "rationaleClassifications not found in withheld information item"
    assert len(bt_710_item["rationaleClassifications"]) > 0, "No rationale classifications found"

    classification = bt_710_item["rationaleClassifications"][0]
