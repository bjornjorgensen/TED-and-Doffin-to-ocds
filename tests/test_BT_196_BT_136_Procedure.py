# tests/test_BT_196_BT_136_Procedure.py
...
def test_bt_196_bt_136_procedure_integration(tmp_path):
    xml_content = """
    ...
    """
    xml_file = tmp_path / "test_input_bt_196_bt_136_procedure.xml"
    ...
    dir_awa_jus_item = next((item for item in result["withheldInformation"] if item.get("id", "").startswith("dir-awa-jus-")), None)
    assert dir_awa_jus_item is not None, "No withheld information item for direct award justification found"
    assert "rationale" in dir_awa_jus_item, "rationale not found in withheld information item"
    assert dir_awa_jus_item["rationale"] == "Information delayed publication because of ...", f"Expected rationale 'Information delayed publication because of ...', got {dir_awa_jus_item['rationale']}"
