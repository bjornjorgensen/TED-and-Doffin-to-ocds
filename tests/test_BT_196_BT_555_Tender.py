...
def test_bt_196_bt_555_tender_integration(tmp_path):
    xml_content = """
    ...
    """
    xml_file = tmp_path / "test_input_bt_196_bt_555_tender.xml"
    ...
    sub_per_item = next((item for item in result["withheldInformation"] if item.get("id") == "sub-per-tender-1"), None)
    assert sub_per_item is not None, "No withheld information item for subcontracting percentage (Tender) found"
    assert "rationale" in sub_per_item, "rationale not found in withheld information item"
    assert sub_per_item["rationale"] == "Information delayed publication because of ...", f"Expected rationale 'Information delayed publication because of ...', got {sub_per_item['rationale']}"
