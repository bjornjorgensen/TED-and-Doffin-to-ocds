...
def test_bt195_bt191_unpublished_identifier_integration(tmp_path, setup_logging):
    ...
    """
    xml_file = tmp_path / "test_input_bt195_bt191.xml"
    ...
    assert (
        withheld_item["name"] == "Country Origin"
    ), "Unexpected withheld information name"


def test_bt195_bt191_unpublished_identifier_missing_data(tmp_path, setup_logging):
    logger = setup_logging
    xml_content = """
    <root xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2"
          xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2"
    ...
    """
    xml_file = tmp_path / "test_input_bt195_bt191_missing.xml"
    xml_file.write_text(xml_content)
