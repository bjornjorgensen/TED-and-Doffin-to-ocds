...
def test_bt_197_bt_734_lotsgroup_integration(tmp_path):
    xml_content = """
    ...
    """
    xml_file = tmp_path / "test_input_bt_197_bt_734_lotsgroup.xml"
    ...
    bt_734_lotsgroup_item = next((item for item in result["withheldInformation"] if item.get("id") == "BT-195(BT-734)-LotsGroup"), None)
    assert bt_734_lotsgroup_item is not None, "No withheld information item for BT-195(BT-734)-LotsGroup found"

    assert "rationaleClassifications" in bt_734_lotsgroup_item, "rationaleClassifications not found in withheld information item"
    assert len(bt_734_lotsgroup_item["rationaleClassifications"]) > 0, "No rationale classifications found"

    classification = bt_734_lotsgroup_item["rationaleClassifications"][0]
