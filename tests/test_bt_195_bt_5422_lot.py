...
def test_bt195_bt5422_lot_integration(tmp_path, setup_logging, temp_output_dir):
    ...
    xml_file = tmp_path / "test_input_bt195_bt5422_lot.xml"
    ...
    assert (
        withheld_item["id"] == "awa-cri-fix-LOT-0001"
...
def test_bt195_bt5422_lot_missing_data(tmp_path, setup_logging, temp_output_dir):
    logger = setup_logging
    xml_content = """<?xml version="1.0" encoding="UTF-8"?>
    <ContractNotice xmlns="urn:oasis:names:specification:ubl:schema:xsd:ContractNotice-2"
    xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2"
    ...
    xml_file = tmp_path / "test_input_bt195_bt5422_lot_missing.xml"
    xml_file.write_text(xml_content)
