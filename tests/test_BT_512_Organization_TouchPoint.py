...
def test_bt_512_organization_touchpoint_integration(tmp_path):
    xml_content = """
    ...
    assert party["address"]["postalCode"] == "2345", f"Expected postal code '2345', got {party['address']['postalCode']}"
    assert "identifier" in party, "Expected 'identifier' in party"
    assert party["identifier"]["id"] == "998298", f"Expected company ID '998298', got {party['identifier']['id']}"
    assert party["identifier"]["scheme"] == "internal", f"Expected scheme 'internal', got {party['identifier']['scheme']}"
