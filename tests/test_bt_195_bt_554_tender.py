...
def test_bt_195_bt554_tender_integration(tmp_path, setup_logging, temp_output_dir):
    ...
    assert item["field"] == "sub-des", f"Expected field 'sub-des', got {item['field']}"
    assert (
        item["name"] == "Subcontracting Description"
    ), f"Expected name 'Subcontracting Description', got {item['name']}"


def test_bt_195_bt554_tender_missing_data(tmp_path, setup_logging, temp_output_dir):
    logger = setup_logging
    xml_content = """<?xml version="1.0" encoding="UTF-8"?>
    <ContractNotice xmlns="urn:oasis:names:specification:ubl:schema:xsd:ContractNotice-2"
    xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2"
