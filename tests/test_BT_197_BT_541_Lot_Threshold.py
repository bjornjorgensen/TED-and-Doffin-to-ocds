...
def test_bt_197_bt_541_lot_threshold_integration(tmp_path):
    xml_content = """
    ...
    """
    xml_file = tmp_path / "test_input_bt_197_bt_541_lot_threshold.xml"
    ...
    bt_541_item = next((item for item in result["withheldInformation"] if item.get("id") == "BT-195(BT-541)-Lot-Threshold"), None)
    assert bt_541_item is not None, "No withheld information item for BT-195(BT-541)-Lot-Threshold found"
