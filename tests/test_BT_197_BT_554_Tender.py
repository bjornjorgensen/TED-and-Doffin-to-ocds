...
def test_bt_197_bt_554_tender_integration(tmp_path):
    xml_content = """
    ...
    """
    xml_file = tmp_path / "test_input_bt_197_bt_554_tender.xml"
    ...
    bt_554_item = next((item for item in result["withheldInformation"] if item.get("id") == "BT-195(BT-554)-Tender"), None)
    assert bt_554_item is not None, "No withheld information item for BT-195(BT-554)-Tender found"

    assert "rationaleClassifications" in bt_554_item, "rationaleClassifications not found in withheld information item"
    assert len(bt_554_item["rationaleClassifications"]) > 0, "No rationale classifications found"

    classification = bt_554_item["rationaleClassifications"][0]
