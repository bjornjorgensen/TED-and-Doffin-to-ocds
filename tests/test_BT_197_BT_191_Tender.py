...
def test_bt_197_bt_191_tender_integration(tmp_path):
    xml_content = """
    ...
    xml_file = tmp_path / "test_input_bt_197_bt_191_tender.xml"
    ...
    bt_191_item = next((item for item in result["withheldInformation"] if item.get("field") == "BT-191"), None)
    assert bt_191_item is not None, "No withheld information item for BT-191 found"

    assert "rationaleClassifications" in bt_191_item, "rationaleClassifications not found in withheld information item"
    assert len(bt_191_item["rationaleClassifications"]) > 0, "No rationale classifications found"

    classification = bt_191_item["rationaleClassifications"][0]
