...
def test_bt_722_contract_integration(tmp_path, setup_logging, temp_output_dir) -> None:
    ...
    ), f"Expected 1 finance object, got {len(contract['finance'])}"
    assert (
        contract["finance"][0]["title"] == "ABC123"
    ), f"Expected finance title 'ABC123', got {contract['finance'][0]['title']}"
    assert (
