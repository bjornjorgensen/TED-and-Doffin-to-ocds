...
def test_bt_805_lot_integration(tmp_path, setup_logging):
    ...
    assert len(lot["sustainability"]) == 2, f"Expected 2 sustainability entries, got {len(lot['sustainability'])}"
    
    strategies = [item for sustainability in lot["sustainability"] for item in sustainability["strategies"]]
    assert "euGPPCriteria" in strategies, "Expected 'euGPPCriteria' in strategies"
    assert "nationalGPPCriteria" in strategies, "Expected 'nationalGPPCriteria' in strategies"

    # Check that LOT-0002 is not included
    assert all(lot["id"] != "LOT-0002" for lot in result["tender"]["lots"]), "LOT-0002 should not be included"
