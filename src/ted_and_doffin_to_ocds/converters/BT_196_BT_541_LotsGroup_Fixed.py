# converters/BT_196_BT_541_LotsGroup_Fixed.py
...
def parse_bt196_bt541_lotsgroup_fixed_unpublished_justification(xml_content):
    """
    Parse the XML content to extract the unpublished justification description for the LotsGroup fixed award criterion.
    ...
    for lots_group in lots_groups:
        ...
        reason_description = lots_group.xpath(
            ".//efac:AwardCriterionParameter[efbc:ParameterCode/@listName='number-fixed']/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='awa-cri-num']/efbc:ReasonDescription/text()",
...
def merge_bt196_bt541_lotsgroup_fixed_unpublished_justification(
    ...
    if not unpublished_justification_data:
        logger.warning(
            "No unpublished justification data to merge for BT-196(BT-541) LotsGroup Fixed",
        )
        return
    ...
    logger.info(
        f"Merged unpublished justification data for BT-196(BT-541) LotsGroup Fixed for {len(unpublished_justification_data['withheldInformation'])} lots groups",
    )
