...
def parse_bt196_bt1351_unpublished_justification(xml_content):
    """
    ...
    reason_description = root.xpath(
        "//cac:TenderingProcess/cac:ProcessJustification[cbc:ProcessReasonCode/@listName='accelerated-procedure']/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='pro-acc-jus']/efbc:ReasonDescription/text()",
        namespaces=namespaces,
    )

    if reason_description:
        withheld_info = {"field": "pro-acc-jus", "rationale": reason_description[0]}
...
def merge_bt196_bt1351_unpublished_justification(
    ...
    if not unpublished_justification_data:
        logger.warning("No unpublished justification data to merge for BT-196(BT-1351)")
        return
    ...
    logger.info("Merged unpublished justification data for BT-196(BT-1351)")
