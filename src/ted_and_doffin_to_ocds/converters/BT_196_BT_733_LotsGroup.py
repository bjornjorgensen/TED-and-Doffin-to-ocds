...
def parse_bt196_bt733_lotsgroup_unpublished_justification(xml_content):
    """
    Parse the XML content to extract the unpublished justification description for the award criteria order justification in LotsGroup.
    ...
    for lots_group in lots_groups:
        ...
        if fields_privacy:
            ...
            if reason_description and group_id:
                withheld_info = {
...
def merge_bt196_bt733_lotsgroup_unpublished_justification(
    ...
    logger.info(
        f"Merged {len(unpublished_justification_data['withheldInformation'])} unpublished justification(s) for BT-196(BT-733)-LotsGroup"
    )
