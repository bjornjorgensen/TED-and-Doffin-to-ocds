...
def parse_organization_touchpoint_email(xml_content):
...
def merge_organization_touchpoint_email(
    release_json, organization_touchpoint_email_data
) -> None:
    if not organization_touchpoint_email_data:
        logger.info("No organization touchpoint email data to merge")
        return

    existing_parties = release_json.setdefault("parties", [])

    for new_party in organization_touchpoint_email_data["parties"]:
        existing_party = next(
    ...
    logger.info(
        "Merged organization touchpoint email data for %d parties",
        len(organization_touchpoint_email_data["parties"]),
    )
