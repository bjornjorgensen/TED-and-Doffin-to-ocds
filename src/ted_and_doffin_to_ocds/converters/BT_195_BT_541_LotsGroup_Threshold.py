...
def merge_bt195_bt541_lotsgroup_threshold_unpublished_identifier(
    ...
    if not unpublished_identifier_data:
        logger.warning(
            "No unpublished identifier data to merge for BT-195(BT-541) LotsGroup Threshold"
        )
        return
    ...
    logger.info(
        f"Merged unpublished identifier data for BT-195(BT-541) LotsGroup Threshold for {len(unpublished_identifier_data['withheldInformation'])} lots groups"
    )
