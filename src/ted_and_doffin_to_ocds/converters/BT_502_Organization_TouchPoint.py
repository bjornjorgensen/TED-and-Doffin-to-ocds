...
def parse_touchpoint_contact_point(xml_content):
    ...
    for organization in organizations:
        ...
        contact_name = organization.xpath(
            "efac:TouchPoint/cac:Contact/cbc:Name/text()", namespaces=namespaces,
        )
        ...
        if touchpoint_id and contact_name:
            party = {"id": touchpoint_id[0], "contactPoint": {"name": contact_name[0]}}
            if company_id:
...
def merge_touchpoint_contact_point(release_json, touchpoint_contact_point_data):
    if not touchpoint_contact_point_data:
        logger.warning("No TouchPoint Contact Point data to merge")
        return
    ...
    logger.info(
        f"Merged TouchPoint Contact Point data for {len(touchpoint_contact_point_data['parties'])} parties",
    )
