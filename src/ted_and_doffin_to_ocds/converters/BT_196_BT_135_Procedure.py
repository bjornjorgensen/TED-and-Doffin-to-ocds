# converters/BT_196_BT_135_Procedure.py
...
def parse_bt196_bt135_unpublished_justification(xml_content):
    """
    ...
    reason_description = root.xpath(
        "//cac:TenderingProcess/cac:ProcessJustification[cbc:ProcessReasonCode/@listName='direct-award-justification']/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='dir-awa-tex']/efbc:ReasonDescription/text()",
        namespaces=namespaces,
    )

    if reason_description:
        withheld_info = {"field": "dir-awa-tex", "rationale": reason_description[0]}
        result["withheldInformation"].append(withheld_info)

    return result if result["withheldInformation"] else None


def merge_bt196_bt135_unpublished_justification(
    ...
    if not unpublished_justification_data:
        logger.warning("No unpublished justification data to merge for BT-196(BT-135)")
        return
    ...
    logger.info("Merged unpublished justification data for BT-196(BT-135)")
