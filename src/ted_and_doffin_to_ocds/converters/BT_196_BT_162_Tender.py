# converters/BT_196_BT_162_Tender.py
...
def parse_bt196_bt162_unpublished_justification(xml_content):
    """
    Parse the XML content to extract the unpublished justification description for the concession revenue user.
    ...
    for lot_tender in lot_tenders:
        ...
        reason_description = lot_tender.xpath(
            "efac:ConcessionRevenue/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='con-rev-use']/efbc:ReasonDescription/text()",
            namespaces=namespaces,
        )

        if lot_tender_id and reason_description:
            withheld_info = {
                "id": f"con-rev-use-{lot_tender_id[0]}",
                "field": "con-rev-use",
...
def merge_bt196_bt162_unpublished_justification(
    release_json,
    ...
    if not unpublished_justification_data:
        logger.warning("No unpublished justification data to merge for BT-196(BT-162)")
        return
    ...
    logger.info(
        f"Merged unpublished justification data for BT-196(BT-162): {len(unpublished_justification_data['withheldInformation'])} items",
    )
