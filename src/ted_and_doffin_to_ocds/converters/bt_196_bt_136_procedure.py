...
def parse_bt196_bt136_unpublished_justification(xml_content):
    """
    ...
    reason_description = root.xpath(
        "//cac:TenderingProcess/cac:ProcessJustification[cbc:ProcessReasonCode/@listName='direct-award-justification']/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='dir-awa-jus']/efbc:ReasonDescription/text()",
        namespaces=namespaces,
    )

    if reason_description:
        withheld_info = {"field": "dir-awa-jus", "rationale": reason_description[0]}
        result["withheldInformation"].append(withheld_info)

    return result if result["withheldInformation"] else None


def merge_bt196_bt136_unpublished_justification(
    release_json,
    ...
    if not unpublished_justification_data:
        logger.warning("No unpublished justification data to merge for BT-196(BT-136)")
        return
    ...
    logger.info("Merged unpublished justification data for BT-196(BT-136)")
