# converters/bt_196_bt_539_lotsgroup.py
...
def parse_bt196_bt539_lotsgroup_unpublished_justification(xml_content):
    """
    Parse the XML content to extract the unpublished justification description for the award criterion type in LotsGroup.
...
def merge_bt196_bt539_lotsgroup_unpublished_justification(
    release_json,
    ...
    if not unpublished_justification_data:
        logger.warning(
            "No unpublished justification data to merge for BT-196(BT-539)-LotsGroup",
        )
        return
    ...
    logger.info(
        f"Merged unpublished justification data for BT-196(BT-539)-LotsGroup: {len(unpublished_justification_data['withheldInformation'])} items",
    )
