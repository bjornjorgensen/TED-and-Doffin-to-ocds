# converters/bt_196_bt_541_lotsgroup_weight.py
...
def parse_bt196_bt541_lotsgroup_weight(xml_content):
    """
    Parse the XML content to extract the unpublished justification description for the lots group weight.
    ...
    for description in reason_descriptions:
        withheld_info = {"field": "awa-cri-num", "rationale": description.text}
        result["withheldInformation"].append(withheld_info)

    return result if result["withheldInformation"] else None


def merge_bt196_bt541_lotsgroup_weight(release_json, unpublished_justification_data):
    """
    ...
    if not unpublished_justification_data:
        logger.warning(
            "No unpublished justification description data to merge for BT-196(BT-541)-LotsGroup-Weight",
        )
        return
    ...
    logger.info(
        "Merged unpublished justification description data for BT-196(BT-541)-LotsGroup-Weight",
    )
