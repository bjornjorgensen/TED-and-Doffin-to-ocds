...
def parse_bt195_bt636_unpublished_identifier(xml_content):
    """
    Parse the XML content to extract the unpublished identifier for the buyer review request irregularity type.
...
def merge_bt195_bt636_unpublished_identifier(release_json, unpublished_identifier_data):
    """
    ...
    if not unpublished_identifier_data:
        logger.warning("No unpublished identifier data to merge for BT-195(BT-636)")
        return
    ...
    logger.info(
        f"Merged {len(unpublished_identifier_data['withheldInformation'])} unpublished identifier(s) for BT-195(BT-636)",
    )
