# converters/bt_196_bt_191_Tender.py
...
def parse_bt196_bt191_unpublished_justification(xml_content):
    """
    Parse the XML content to extract the unpublished justification description for the country origin.
    ...
    for lot_tender in lot_tenders:
        ...
        reason_description = lot_tender.xpath(
            "efac:Origin/efac:FieldsPrivacy[efbc:FieldIdentifierCode/text()='cou-ori']/efbc:ReasonDescription/text()",
            namespaces=namespaces,
        )
...
def merge_bt196_bt191_unpublished_justification(
    release_json,
    ...
    if not unpublished_justification_data:
        logger.warning("No unpublished justification data to merge for BT-196(BT-191)")
        return
    ...
    logger.info(
        f"Merged unpublished justification data for BT-196(BT-191): {len(unpublished_justification_data['withheldInformation'])} items",
    )
