...
def parse_lot_eu_funds(xml_content):
    ...
    for lot in lots:
        ...
        funding_programs = lot.xpath(
            "cac:TenderingTerms/ext:UBLExtensions/ext:UBLExtension/ext:ExtensionContent/efext:EformsExtension/efac:Funding/cbc:FundingProgramCode[@listName='eu-programme']/text()",
            namespaces=namespaces,
        )
        ...
        if lot_id and funding_programs:
            lot_data = {
                ...
                "planning": {
                    "budget": {
                        "finance": [
                            {"id": str(i + 1), "title": program}
                            for i, program in enumerate(funding_programs)
                        ],
                    },
                },
            }
        ...
        if existing_lot:
            ...
            for new_finance in new_lot["planning"]["budget"]["finance"]:
                existing_finance = next(
                    (
                        f
                        for f in existing_lot["planning"]["budget"]["finance"]
                        if f["id"] == new_finance["id"]
                    ),
                    None,
                )
                if existing_finance:
