...
def parse_electronic_auction_description(xml_content: str | bytes) -> dict | None:
    """Parse electronic auction description from XML for each lot.
...
def merge_electronic_auction_description(
    ...
    for new_lot in auction_description_data["tender"]["lots"]:
        existing_lot = next(
        ...
        if existing_lot:
            ...
            electronic_auction["description"] = new_lot["techniques"][
                "electronicAuction"
            ]["description"]
        else:
    ...
    logger.info(
        ...
        len(auction_description_data["tender"]["lots"]),
    )
