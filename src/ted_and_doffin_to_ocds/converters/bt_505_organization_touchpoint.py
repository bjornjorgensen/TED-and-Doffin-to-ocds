...
def parse_organization_touchpoint_website(xml_content):
...
def merge_organization_touchpoint_website(
    release_json, organization_touchpoint_website_data
):
    if not organization_touchpoint_website_data:
        logger.info("No organization touchpoint website data to merge")
        return

    existing_parties = release_json.setdefault("parties", [])

    for new_party in organization_touchpoint_website_data["parties"]:
        existing_party = next(
    ...
    logger.info(
        "Merged organization touchpoint website data for %d parties",
        len(organization_touchpoint_website_data["parties"]),
    )
