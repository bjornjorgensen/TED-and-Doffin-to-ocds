...
def parse_bt196_bt733_unpublished_justification(xml_content):
    """
...
def merge_bt196_bt733_unpublished_justification(
    ...
    if not unpublished_justification_data:
        logger.warning("No unpublished justification data to merge for BT-196(BT-733)")
        return
    ...
    logger.info(
        f"Merged {len(unpublished_justification_data['withheldInformation'])} unpublished justification(s) for BT-196(BT-733)"
    )
