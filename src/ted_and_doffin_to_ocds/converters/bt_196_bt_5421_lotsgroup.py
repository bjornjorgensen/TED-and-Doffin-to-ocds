# converters/bt_196_bt_5421_lotsgroup.py
...
def parse_bt196_bt5421_lotsgroup(xml_content):
    """
    ...
    for description in reason_descriptions:
        withheld_info = {"field": "awa-cri-wei", "rationale": description.text}
        result["withheldInformation"].append(withheld_info)

    return result if result["withheldInformation"] else None


def merge_bt196_bt5421_lotsgroup(release_json, unpublished_justification_data) -> None:
    """
    ...
    if not unpublished_justification_data:
        logger.warning(
            "No unpublished justification description data to merge for BT-196(BT-5421)-LotsGroup",
        )
        return
    ...
    logger.info(
        "Merged unpublished justification description data for BT-196(BT-5421)-LotsGroup",
    )
