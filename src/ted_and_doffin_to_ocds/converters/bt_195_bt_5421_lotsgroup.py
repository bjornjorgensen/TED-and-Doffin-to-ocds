...
def parse_bt195_bt5421_lotsgroup(xml_content):
    """
    Parse the XML content to extract the unpublished identifier for the lots group.
...
def merge_bt195_bt5421_lotsgroup(release_json, unpublished_identifier_data) -> None:
    """
    ...
    if not unpublished_identifier_data:
        logger.warning(
            "No unpublished identifier data to merge for BT-195(BT-5421)-LotsGroup",
        )
        return
    ...
    logger.info("Merged unpublished identifier data for BT-195(BT-5421)-LotsGroup")
